...
All Stage C implementation helpers (closure building, parameter initialization, LBFGS execution,
and warm-cache detector retargeting) now live as private methods within the StageC class
per ARCH-REFACTOR-001 Phase C.
...
- dbex.refinement.config_factories: Factory functions for detector/crystal config
- dbex.nanobrag_bridge: Baseline misset computation
...
class StageC:
    ...
    def _retarget_stage_a_detectors(
        ...
        debug_path = os.environ.get("DBEX_STAGE_C_CACHE_DEBUG_PATH", None)
        debug_enabled = debug_path is not None
        debug_data = None

        if debug_enabled:
            # Use class attribute for counter (initialize if not present)
            if not hasattr(StageC, '_debug_call_counter'):
                StageC._debug_call_counter = 0
            call_idx = StageC._debug_call_counter
            StageC._debug_call_counter += 1
            debug_data = {
                "call_index": call_idx,
                "panel_updates": [],
                "roi_updates": [],
    ...
    def _build_lbfgs_closure(
        ...
        def compute_loss_stage_c(panel_ids: List[int], is_full: bool = False, force_panel_eval: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
            ...
            if stage_c_use_warm_cache:
                ...
                StageC._retarget_stage_a_detectors(
