...
| Stage A/B/C refinement smokes | `AUTHORITATIVE_CMDS_DOC=./docs/TESTING_GUIDE.md KMP_DUPLICATE_LIB_OK=TRUE NANOBRAGG_DISABLE_COMPILE=1 pytest -v tests/dbex/test_torch_refine_smoke.py -k 'test_stage_a_expansion or test_stage_b_shell_modifiers or test_stage_c_detector_microslip' --smoke-detector-size=small` | Active | `docs/spec-db-workflow.md` (“Stage Smoke Dataset Policy”) | Exercises Stage A/B/C LBFGS closures on the cropped `sp.proc/refGeom_small/` assets (29 ROIs) with relaxed gates; telemetry (perf counters, loss traces, ROI counts) is appended to `$DBEX_SMOKE_TELEMETRY_PATH` for perf tracking. Set `--smoke-detector-size=full` or `DBEX_SMOKE_DETECTOR_SIZE=full` when parity thresholds are required (DB‑AT/DB‑PARITY selectors). A global pytest guard (`tests/conftest.py::pytest_runtest_setup`) enforces this switch for DB-AT/workflow selectors per `docs/spec-db-workflow.md`. Artifacts: `plans/active/PERF-SMOKE-DETSIZE/reports/<timestamp>/collect_stage_a_small.log`, `pytest_stage_smokes_small.log`, `telemetry_small.json`. |
