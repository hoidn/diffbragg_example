...
| Mapping consistency | `KMP_DUPLICATE_LIB_OK=TRUE DBAT024_ARTIFACT_DIR=plans/active/DB-AT-024/reports/<timestamp> pytest -v tests -k DB_AT_024` | Zero-iteration mapping consistency guard per `docs/spec-db-conformance.md:43-46`. Validates ROI correlation ≥0.2 (median) and localization success rate ≥90% between torch Bragg output and background-subtracted data using `simulate_forward_once` helper. Tests emit per-ROI metrics (correlation, RMSE, localization) and diagnostics (masked_mse, loss_mask_coverage, bragg_stats, hkl_stats) under `$DBAT024_ARTIFACT_DIR`. Provisional xfail until thresholds met (baseline: corr_median=0.0488, localization_success_rate=0.0 from 2025-11-04T063053Z). | Active | Tests: `tests/dbex/test_mapping_consistency.py` (1 test collected, 1 xfailed). Canonical refGeom: 92 ROIs, corr_median=0.0488, localization_success_rate=0.0, global_scale_hint=62.66, masked_mse=981700.05. Skip guard active when refGeom assets absent or `DBAT024_ARTIFACT_DIR` unset. Artifacts: `plans/active/DB-AT-024/reports/2025-11-04T070000Z/` (pytest_db_at_024.log, collect_db_at_024.log, mapping_metrics.json, mapping_metrics.csv). Findings: CONFORMANCE-001, TESTING-003, PARITY-001, CONFIG-001, SCALE-002, RUNTIME-001. Environment: `KMP_DUPLICATE_LIB_OK=TRUE`, `DBAT024_ARTIFACT_DIR=plans/active/DB-AT-024/reports/<timestamp>`, `NANOBRAGG_DISABLE_COMPILE=1` (recommended). |
