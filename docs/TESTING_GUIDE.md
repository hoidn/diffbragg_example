...
**Stage smoke dataset selection (optional):**
```bash
...
- **Scope**: Small is the default for smoke/perf loops. DB-AT and parity selectors MUST set this to `full` (or pass the pytest option) before running Stage smokes to ensure error budgets match the spec.
