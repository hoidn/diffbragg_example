...
| Torch parity | `KMP_DUPLICATE_LIB_OK=TRUE pytest -v tests -k DB_AT_001` | Validates PyTorch parity against the designated reference dataset once published (`docs/spec-db-conformance.md:24`). | Planned | Harness spec: `docs/parity_harness_spec.md` §2. No canonical dataset is currently available; selector MUST xfail/skip with reason `parity dataset unavailable` and record attempted manifest lookup. When data lands, require correlation ≥0.99, MSE, RMSE, max\|Δ\|, sum_ratio. Artifacts: `plans/active/<initiative>/reports/<timestamp>/parity/` (metrics.json, traces, heatmaps). Trace workflow per `docs/spec-db-tracing.md:10-26`. |
