...
| Reflection ingestion (DB_AT_020) | `pytest -v tests -k DB_AT_020 --smoke-detector-size=full` | DIALS reflection ingestion + bbox semantics (`docs/spec-db-conformance.md:28-30`). Validates bbox exclusivity (x1 > x0, y1 > y0), slice shape consistency, panel alignment, and detector bounds for all ROIs. | Active | Tests: `tests/dbex/test_reflection_ingestion.py` (2 tests collected: `test_DB_AT_020_reflection_bbox`, `test_DB_AT_020_panel_alignment`). Canonical: 92 ROIs, 1 panel, sample bbox=(582,594,0,12). Runtime: ~1.2s. Skip guard: test skips when `refGeom.refl` missing (mirrors smoke fixture guard per `tests/dbex/conftest.py::refgeom_dataload`). Environment: `DBEX_SMOKE_DETECTOR_SIZE=full`, `KMP_DUPLICATE_LIB_OK=TRUE`. Canonical command: `DBEX_SMOKE_DETECTOR_SIZE=full AUTHORITATIVE_CMDS_DOC=./docs/TESTING_GUIDE.md KMP_DUPLICATE_LIB_OK=TRUE pytest -vv tests/dbex/test_reflection_ingestion.py::TestReflectionIngestion::test_DB_AT_020_reflection_bbox`. Artifacts: `plans/active/DB-AT-020/reports/2025-12-08T100000Z/` (pytest_db_at_020_regression.log, collect_db_at_020.log). Findings: CONFORMANCE-001, TESTING-003, MASKING-001. |
| Mask semantics (DB_AT_021) | `DBEX_SMOKE_USE_GOLDEN_SIMPLE_CUBIC=1 DBEX_SMOKE_DETECTOR_SIZE=full AUTHORITATIVE_CMDS_DOC=./docs/TESTING_GUIDE.md KMP_DUPLICATE_LIB_OK=TRUE pytest -vv tests/dbex/test_mask_semantics.py::TestDB_AT_021_MaskSemantics -k DB_AT_021` | Trusted-mask polarity, loss_mask construction, and ARCH-CONTRACT-MASKING-001 precedence per `docs/spec-db-core.md:124`. Validates mask polarity (True=trusted per DIALS convention), loss_mask formula `(background >= 0) & trusted_mask`, and canonical owner enforcement (no duplicate loss_mask construction in Stage helpers). | Active | Tests: `tests/dbex/test_mask_semantics.py::TestDB_AT_021_MaskSemantics` (3 tests: `test_DB_AT_021_polarity_checks`, `test_DB_AT_021_loss_mask_construction`, `test_DB_AT_021_precedence_guards`). Canonical metrics (Phase B, 2025-12-08T150000Z): trusted_pixels=5,696,996 (91.5%), loss_mask_pixels=13,084 (0.2% coverage — sparse Bragg peaks expected per MASKING-001), duplicates_found=0 (ARCH-CONTRACT-MASKING-001 enforcement). Runtime: ~4s. Skip guard: test skips when `refGeom.expt`/`refGeom.refl` missing (mirrors smoke fixture guard). Environment: `DBEX_SMOKE_USE_GOLDEN_SIMPLE_CUBIC=1`, `DBEX_SMOKE_DETECTOR_SIZE=full`, `KMP_DUPLICATE_LIB_OK=TRUE`. Artifacts: `plans/active/DB-AT-021/reports/2025-12-08T170000Z/` (pytest_db_at_021_regression.log, collect_db_at_021.log). Findings: MASKING-001 (sparse coverage expected), TESTING-003 (selector compliance), CONFORMANCE-001 (DB-AT naming), DIALS-API polarity (True=trusted). First added 2025-12-08 (Phase B: i=149). |
| Background semantics | `KMP_DUPLICATE_LIB_OK=TRUE DBEX_SMOKE_DETECTOR_SIZE=full pytest -vv tests/dbex/test_background_semantics.py -k DB_AT_022` | Validates −1 sentinel handling around ROIs per `docs/simtbx_api.md:14` and `docs/spec-db-workflow.md:38`. Enforces sentinel integrity guard in `prepare_refinement_inputs`: background <= -0.5 outside ROI union, no sentinel inside ROIs. Tests sentinel complement, guard enforcement with tampered data, and ROI coverage metrics. | Active | Tests: `tests/dbex/test_background_semantics.py::TestBackgroundSemantics` (3 tests collected, 3 passed: `test_DB_AT_022_sentinel_complement`, `test_DB_AT_022_guard_enforcement`, `test_DB_AT_022_roi_coverage_metrics`). Canonical: sentinel_fraction=99.8%, roi_fraction=0.2%, overlap=0, sentinel_mean=-1.0, 92 ROIs. Skip guard active when refGeom assets absent. Environment: `KMP_DUPLICATE_LIB_OK=TRUE`, `DBEX_SMOKE_DETECTOR_SIZE=full`. Canonical command: `KMP_DUPLICATE_LIB_OK=TRUE DBEX_SMOKE_DETECTOR_SIZE=full pytest -vv tests/dbex/test_background_semantics.py -k DB_AT_022`. Artifacts: `plans/active/DB-AT-022/reports/2025-12-08T200000Z/` (pytest_db_at_022.log, collect_db_at_022.log). Applied findings: MASKING-001 (sentinel exclusion via `background >= 0` guard), TESTING-003, DIAGNOSTICS-001. First added 2025-11-04; updated 2025-12-08 (Phase B.3 + C closure, Loop i=152). |
...
| Stage A Zero-Point Parity | `AUTHORITATIVE_CMDS_DOC=./docs/TESTING_GUIDE.md DBEX_SMOKE_SIGMA_SOURCE=metadata DBEX_SMOKE_DETECTOR_SIZE=full DBAT027_ARTIFACT_DIR=plans/active/TOOLING-VIS-001/reports/<timestamp>/db_at_027 KMP_DUPLICATE_LIB_OK=TRUE NANOBRAGG_DISABLE_COMPILE=1 pytest -vv tests/dbex/test_stage_a_mapping_equiv.py::test_db_at_027_zero_point_parity` | DB-AT-027: Stage A zero-point mapping equivalence per `docs/spec-db-conformance.md:201-239`. Validates Stage A zero-parameter forward model equals DB-AT-024 mapping at same geometry/HKL/calibration. Tolerances: mean_abs_diff ≤ 1e-3, max_abs_diff ≤ 2.0e2, \|chi2_rel_diff\| ≤ 1e-3. Uses canonical variance-weighted loss (PHYSICS-LOSS-001). | Active | Status: Active. Calibration (spot_scale_override + flux/exposure/beamsize/N_cells) now threads through Stage A warm cache and reconstruction; log_scale baseline recorded in telemetry. Latest run (2025-11-24T234121Z) meets tolerances: mean_abs_diff≈1.2e-07, max_abs_diff≈2.34e-02, chi2_rel_diff≈−8.4e-09. Artifacts: `plans/active/TOOLING-VIS-001/reports/2025-11-24T234121Z/db_at_027/{db_at_027_metrics.json,db_at_027_env.json,pytest_db_at_027.log,pytest_db_at_027_collect.log}`. Findings: STAGEA-001 resolved (calibration plumbed), PHYSICS-LOSS-001, CONFORMANCE-001. Environment: `AUTHORITATIVE_CMDS_DOC`, `DBEX_SMOKE_SIGMA_SOURCE=metadata`, `DBEX_SMOKE_DETECTOR_SIZE=full`, `DBAT027_ARTIFACT_DIR`, `KMP_DUPLICATE_LIB_OK=TRUE`, `NANOBRAGG_DISABLE_COMPILE=1`. |
| Stage A Loss-Scale Sanity | `AUTHORITATIVE_CMDS_DOC=./docs/TESTING_GUIDE.md DBEX_SMOKE_SIGMA_SOURCE=metadata DBEX_SMOKE_DETECTOR_SIZE=small DBAT028_ARTIFACT_DIR=plans/active/TOOLING-VIS-001/reports/2025-11-25T005459Z/db_at_028 DBAT029_ARTIFACT_DIR=plans/active/TOOLING-VIS-001/reports/2025-11-25T005459Z/db_at_029 KMP_DUPLICATE_LIB_OK=TRUE NANOBRAGG_DISABLE_COMPILE=1 pytest -vv tests/dbex/test_stage_a_smoke_parity.py -k "DB_AT_028"` | DB-AT-028: chi²-per-pixel bounds and clamp fraction on Stage A smoke (nearest-neighbor HKL per `docs/spec-db-workflow.md`). Asserts χ²/pixel initial/final ≤ 1e2, non-increasing χ²/pixel, clamp_fraction < 0.5; writes metrics JSON to `$DBAT028_ARTIFACT_DIR`. | Active | Artifacts: `plans/active/TOOLING-VIS-001/reports/2025-11-25T005459Z/db_at_028/db_at_028_metrics.json`, shared pytest log `plans/active/TOOLING-VIS-001/reports/2025-11-25T005459Z/pytest_db_at_028_029.log`. Uses calibration metadata (config_torch.json) and metadata sigma tiles. Findings: PHYSICS-LOSS-001/002, CONFORMANCE-001. |
| Stage A Structure Parity | `AUTHORITATIVE_CMDS_DOC=./docs/TESTING_GUIDE.md DBEX_SMOKE_SIGMA_SOURCE=metadata DBEX_SMOKE_DETECTOR_SIZE=small DBAT028_ARTIFACT_DIR=plans/active/TOOLING-VIS-001/reports/2025-11-25T005459Z/db_at_028 DBAT029_ARTIFACT_DIR=plans/active/TOOLING-VIS-001/reports/2025-11-25T005459Z/db_at_029 KMP_DUPLICATE_LIB_OK=TRUE NANOBRAGG_DISABLE_COMPILE=1 pytest -vv tests/dbex/test_stage_a_smoke_parity.py -k "DB_AT_029"` | DB-AT-029: ROI correlations and scale sanity vs data on Stage A smoke (nearest-neighbor HKL). Requires median ROI corr before ≥ 0.2, median after ≥ before − 0.05, scale_ratio_before ∈ [1e-2, 1e2]; writes metrics JSON to `$DBAT029_ARTIFACT_DIR`. | Active | Artifacts: `plans/active/TOOLING-VIS-001/reports/2025-11-25T005459Z/db_at_029/db_at_029_metrics.json`, shared pytest log `plans/active/TOOLING-VIS-001/reports/2025-11-25T005459Z/pytest_db_at_028_029.log`. Reuses calibrated reconstruction helper `_build_final_bragg_from_stage_a_telemetry` for bragg_before/after. Findings: PHYSICS-LOSS-001/002/003, CONFORMANCE-001. |
...
| ROI scoring helper | `AUTHORITATIVE_CMDS_DOC=./docs/TESTING_GUIDE.md KMP_DUPLICATE_LIB_OK=TRUE pytest -vv tests/dbex/test_roi_analysis.py` | Validates ROI scoring helper (`dbex/io/roi_scoring.py::score_roi_payloads`) that executes Nelder-Mead optimization with roiCheck objective to produce typed ROIAnalysisPayload instances for writer serialization (ARCH-BRIDGE-RESP-001 Phase B.1). Tests cover: scale recovery (known scale validation), injected checker/log_fn hooks (flexible scoring callback), multiple ROIs with different scales (batch processing), variance floor enforcement (sigma_floor > 0), and sigma parameter validation (sigma_readout >= 0). | Active | Tests: `tests/dbex/test_roi_analysis.py::TestScoreROIPayloads` (5 tests collected, 5 passed). Canonical: optimizer="Nelder-Mead", optimal_scale recovery within 1e-6 for synthetic fixtures, variance_floor applied when sigma_floor > 0. Runtime: ≈1.7s. Skip guard: none (self-contained scipy fixtures). Artifacts: `archive/plans/ARCH-BRIDGE-RESP-001/reports/2025-12-03T051500Z/` (collect_roi_analysis.log, pytest_roi_analysis.log). Findings: DIAGNOSTICS-001 (ROI telemetry schema), PHYSICS-LOSS-001/002/003 (variance model provenance), ARCH-BRIDGE-RESP-001 (writer responsibility split). Environment: `AUTHORITATIVE_CMDS_DOC=./docs/TESTING_GUIDE.md`, `KMP_DUPLICATE_LIB_OK=TRUE`. |
| Runtime vectorization | `KMP_DUPLICATE_LIB_OK=TRUE NANOBRAGG_DISABLE_COMPILE=1 RUNTIME_VEC_ARTIFACT_DIR=plans/active/RUNTIME-VEC-001/reports/<timestamp>/artifacts pytest -v tests/dbex/test_runtime_vectorization.py::TestRuntimeVectorization::test_source_weights_ignored_per_spec` | Equal-weight source handling per `docs/pytorch_runtime_checklist.md:33` and `docs/architecture/pytorch_design.md:90`. Validates source weights are parsed but ignored (correlation ≥0.999, \|sum_ratio−1\| ≤5e-3) via CLI comparison of weighted vs equal sourcefiles. | Active | Tests: `tests/dbex/test_runtime_vectorization.py` (1 test collected, 1 passed). Canonical: correlation=1.0, sum_ratio=1.0, sum_ratio_delta=0.0. Runtime: ≈2.9s (128×128 detector, oversample=1). Artifacts: `plans/active/RUNTIME-VEC-001/reports/2025-12-08T160000Z/` (pytest_runtime_vec.log, collect_runtime_vec.log, artifacts/mapping_metrics.json, artifacts/source_weight_test_summary.txt). Findings: RUNTIME-001, SCALE-001/002, TESTING-003. Environment: `KMP_DUPLICATE_LIB_OK=TRUE`, `NANOBRAGG_DISABLE_COMPILE=1`, `RUNTIME_VEC_ARTIFACT_DIR=plans/active/RUNTIME-VEC-001/reports/<timestamp>/artifacts`. First validated 2025-12-08 (Loop i=165, Phase B complete). |
...
- DB‑AT‑010 uses `simulate_forward_torch` (from `dbex/physics/forward.py`) in float64 with `NANOBRAGG_DISABLE_COMPILE=1`; it is test‑scoped and MUST NOT be reused inside the production LBFGS closure.
- The production refinement loop uses the same masked‑MSE form (via `_compute_variance_weighted_loss` from `dbex/physics/loss.py`) but runs in its own module with the closure rules defined in `docs/spec-db-workflow.md` (Gradient Hygiene).
...
| CLI backend flag | `pytest -v tests/dbex/test_refine_one_cli.py` | Active | `docs/spec-db-interfaces.md:11`, `plans/active/TORCH-CLI-003/implementation.md`, `plans/active/PHYSICS-LOSS-001/implementation.md` | Parser validation, backend dispatch (diffbragg/nanobrag), torch path bridge invocation, diagnostics metadata, and sigma-source guardrails. Includes calibration-positive test (test_nanobrag_backend_applies_calibration) validating `--torch-config`/`--refined-mtz` workflow per SCALE-006, refined MTZ telemetry (`test_nanobrag_backend_uses_refined_mtz`, SCALE-003/004/007), CLI sigma guard (`test_nanobrag_backend_requires_sigma_rdout`), calibrated-map happy path (`test_nanobrag_backend_accepts_sigma_map`), metadata provenance (`test_nanobrag_backend_accepts_external_lookup_sigma_map`), and diagnostics metadata assertions (`test_torch_diagnostics_metadata`). The `test_torch_diagnostics_metadata` selector validates `/torch_diagnostics` telemetry fields including `roi_scoring_method` ("nelder_mead"), `roi_checker` ("score_trainer.roi_check.roiCheck"), `sigma_readout_provenance`, and `sigma_readout_reference_value` per ARCH-BRIDGE-RESP-001 Phase B.3/B.4. Requires `KMP_DUPLICATE_LIB_OK=TRUE`. Collection logs: `plans/active/MAP-SCALE-005/reports/2025-11-06T050000Z/collect_refined_mtz_guard.log` (calibration), `plans/active/PHYSICS-LOSS-001/reports/2025-11-21T063052Z/collect_cli_sigma_map.log` (sigma selectors), and `archive/plans/ARCH-BRIDGE-RESP-001/reports/2025-12-03T051500Z/collect_cli_metadata.log` (ROI payload telemetry). Finding refs: TESTING-002 (mocking strategy), DIAGNOSTICS-001 (HDF5 metadata), TESTING-003 (selector compliance), SCALE-003/004/006/007, PHYSICS-LOSS-001/002/003/005, ARCH-BRIDGE-RESP-001 (writer/bridge responsibility split).
