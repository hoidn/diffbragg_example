...
| Mapping consistency | `KMP_DUPLICATE_LIB_OK=TRUE DBAT024_ARTIFACT_DIR=plans/active/MAP-SCALE-001/reports/<timestamp> NANOBRAGG_DISABLE_COMPILE=1 pytest -v tests -k DB_AT_024` | Zero-iteration mapping consistency guard per `docs/spec-db-conformance.md:43-46`. Validates ROI correlation ≥0.2 (median) and localization success rate ≥90% between torch Bragg output and background-subtracted data using `simulate_forward_once` helper. Tests emit per-ROI metrics (correlation, RMSE, localization) and diagnostics (masked_mse, loss_mask_coverage, bragg_stats, hkl_stats, n_cells_applied) under `$DBAT024_ARTIFACT_DIR`. Test now passes thresholds with calibration metadata and sample clipping (2025-11-04T190041Z). | Active | Tests: `tests/dbex/test_mapping_consistency.py::TestDB_AT_024_Mapping::test_db_at_024_mapping_smoke` (1 test collected, 1 passed). Latest metrics (2025-11-04T190041Z): corr_median=0.621, localization_success_rate=0.935, n_cells_applied=true, calibration.spot_scale_override=3.185e17, calibration.N_cells=[36,28,26], 92 ROIs. Skip guard active when refGeom assets absent or `DBAT024_ARTIFACT_DIR` unset. Artifacts: `plans/active/MAP-SCALE-001/reports/2025-11-04T220000Z/` (mapping_metrics.json, mapping_metrics.csv, pytest.log). Findings: CONFORMANCE-001, TESTING-003, PARITY-001, CONFIG-001, SCALE-002, SCALE-005, RUNTIME-001. Environment: `KMP_DUPLICATE_LIB_OK=TRUE`, `DBAT024_ARTIFACT_DIR=plans/active/MAP-SCALE-001/reports/<timestamp>`, `NANOBRAGG_DISABLE_COMPILE=1` (recommended). |
