...
- The nanobrag CLI SHALL refuse to launch unless a strictly positive detector readout noise source is available via either `--sigma-rdout=<photons>` **or** `--sigma-map=<path>`. Calibrated maps may be `.npy/.npz` stacks shaped `[panel, slow, fast]` or pickled tuples of per-panel numpy/flex arrays; values MUST be in ADU (the CLI converts to photons when `--adu-per-photon` is supplied). Silent fallbacks to zero variance remain prohibited.
- Telemetry: Stage A/B/C groups inside `/torch_diagnostics` now record `sigma_readout_provenance` (e.g., `cli_override`, `calibrated_map`) and `sigma_readout_reference_value` (target units, photons when gains are provided). Downstream tooling should assert these attrs when validating chi-squared traces, especially when calibrated maps are injected.
- Regression coverage: `tests/dbex/test_refine_one_cli.py::{test_nanobrag_backend_requires_sigma_rdout,test_nanobrag_backend_accepts_sigma_map,test_torch_diagnostics_metadata}` exercises the CLI guard, calibrated-map happy path, and telemetry. Loader coverage lives in `tests/dbex/test_data_load_sigma_map.py::test_sigma_map_loader_formats`. Collection logs for these selectors reside under `plans/active/PHYSICS-LOSS-001/reports/2025-11-21T060701Z/` (`collect_cli_sigma_map.log`, `collect_data_load_sigma_map.log`); pytest logs share the same prefix.
...
| CLI backend flag | `pytest -v tests/dbex/test_refine_one_cli.py` | Active | `docs/spec-db-interfaces.md:11`, `plans/active/TORCH-CLI-003/implementation.md`, `plans/active/PHYSICS-LOSS-001/implementation.md` | Parser validation, backend dispatch (diffbragg/nanobrag), torch path bridge invocation, diagnostics metadata, and sigma-source guardrails. Includes calibration-positive test (test_nanobrag_backend_applies_calibration) validating `--torch-config`/`--refined-mtz` workflow per SCALE-006, refined MTZ telemetry (`test_nanobrag_backend_uses_refined_mtz`, SCALE-003/004/007), CLI sigma guard (`test_nanobrag_backend_requires_sigma_rdout`), calibrated-map happy path (`test_nanobrag_backend_accepts_sigma_map`), and diagnostics metadata assertions (`test_torch_diagnostics_metadata`). Requires `KMP_DUPLICATE_LIB_OK=TRUE`. Collection logs: `plans/active/MAP-SCALE-005/reports/2025-11-06T050000Z/collect_refined_mtz_guard.log` (calibration) and `plans/active/PHYSICS-LOSS-001/reports/2025-11-21T060701Z/collect_cli_sigma_map.log` (sigma selectors). Finding refs: TESTING-002 (mocking strategy), DIAGNOSTICS-001 (HDF5 metadata), TESTING-003 (selector compliance), SCALE-003/004/006/007, PHYSICS-LOSS-001/002/003.
| Sigma map ingestion | `pytest -vv tests/dbex/test_data_load_sigma_map.py` | Active | `docs/spec-db-core.md:32-68`, `docs/spec-db-workflow.md:26-31` | Loader helper validates calibrated readout-noise assets (`load_sigma_readout_map`): `.npy/.npz` stacks or pickled tuples of per-panel numpy/flex arrays, strict `[panel, slow, fast]` alignment, and positivity guards. Regression ensures actionable ValueErrors for shape/positivity issues before CLI dispatch. Collection log: `plans/active/PHYSICS-LOSS-001/reports/2025-11-21T060701Z/collect_data_load_sigma_map.log`. Findings: PHYSICS-LOSS-001/002/003, SCALE-007 (actionable error messaging).
