...
- **Canonical full-detector gates**: When `DBEX_SMOKE_DETECTOR_SIZE=full`, Stage C must cut the injected ±0.25 mm offsets by ≥80 % (or end within ±0.05 mm) **and** keep Stage C chi-squared within +0.05 % of Stage A. Stage B must stay within ±1e-6 relative loss while every shell modifier remains within ±1 % of identity. These limits are derived from REFINE-007/008 telemetry on the canonical detector (artifact path: `plans/active/PERF-SMOKE-DETSIZE/reports/2025-11-21T032803Z/`).
