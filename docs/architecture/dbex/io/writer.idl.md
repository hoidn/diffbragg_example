...
| `inputs` | `RefinementInputs` | Yes | Dataclass (from `dbex.refinement.inputs`) with: `target` (torch.Tensor), `loss_mask` (torch.Tensor bool), `panel_slices` (list), `trusted_mask` (per-panel mask) | `dbex.refinement.inputs.prepare_refinement_inputs()` |
