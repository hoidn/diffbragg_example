...
- Preparation: `dbex/refinement/inputs.py::prepare_refinement_inputs` builds background-subtracted targets, loss mask `(background >= 0) & trusted`, ROI slices, sigma tensors, ADU↔photon conversion, global_scale_hint (ADU). RefinementInputs dataclass is defined in the same module.
...
- Configs: Config builders (`create_detector_config`, `create_beam_config`, `create_crystal_config`) now live in `dbex/refinement/config_factories.py`, mapping dxtbx geometry → nanobrag_torch configs per docs/config_crosswalk.md; forward-only simulator instantiation is funneled through the unified factory in `dbex/refinement/helpers.py` (Stage closures keep direct `Simulator` objects for autograd).
- Orchestration: `dbex/nanobrag_bridge.py` retains structure factor grid builders, calibration metadata loading, and HKL helpers; the prep/config responsibilities now live under `dbex/refinement/` as of ARCH-BRIDGE-RESP-001 Phase C.
...
- `dbex.refinement.inputs.prepare_refinement_inputs(data, background_image, trusted_mask, bbox, pids, detector, adu_per_photon=None, sigma_readout=None, sigma_readout_provenance=None)` → `RefinementInputs` dataclass with `target`, `loss_mask`, `panel_slices`, `trusted_mask`, `sigma_readout`, `target_representation`, `global_scale_hint`, `sigma_readout_provenance`.
- Config builders (in `dbex.refinement.config_factories`): `create_detector_config(panel, beam, trusted_mask[, distance_mm_override, roi_bbox])`, `create_beam_config(beam[, flux, beamsize_mm, exposure])`, `create_crystal_config(crystal, experiment[, N_cells, apply_n_cells, crystal_overrides, misset_deg_override])`.
