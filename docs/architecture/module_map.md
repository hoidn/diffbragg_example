...
| `dbex/refinement/inputs.py` | RefinementInputs dataclass, input prep (target/loss mask, background subtraction, ADU↔photon, sigma broadcast) | `RefinementInputs`, `prepare_refinement_inputs` | `test_nanobrag_bridge.py`, `test_mapping_consistency.py` | Active (ARCH-BRIDGE-RESP-001 Phase C) |
| `dbex/refinement/config_factories.py` | Config builders: dxtbx → nanobrag_torch (Detector/Beam/Crystal configs per docs/config_crosswalk.md) | `create_detector_config`, `create_beam_config`, `create_crystal_config` | `test_nanobrag_bridge_configs.py` | Active (ARCH-BRIDGE-RESP-001 Phase C) |
| `dbex/nanobrag_bridge.py` | Orchestration plus HKL grid builders, calibration metadata loading, orientation helpers (recover_cell_from_a_star, quaternion transforms) | `build_structure_factor_grid`, calibration loaders, re-exports (TODO: remove after downstream migration) | `test_nanobrag_bridge*.py` | Active (reduced scope per ARCH-BRIDGE-RESP-001 Phase C; prep/config now in refinement/) |
