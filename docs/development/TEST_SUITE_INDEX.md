...
| Module / Area | Selector | Status | Spec Reference | Notes |
| --- | --- | --- | --- | --- |
| Torch parity | `KMP_DUPLICATE_LIB_OK=TRUE pytest -v tests -k DB_AT_001` | active (partial) | `docs/parity_harness_spec.md:17`, `docs/spec-db-conformance.md:24` | Harness: §2 of parity_harness_spec. FALLBACK dataset available at `tests/fixtures/golden_data/simple_cubic/` (synthetic Bragg, refGeom targets). Manifest integrity tests active (3 tests: checksum validation, tensor shapes, pixel pitch guards). Full parity metrics (correlation ≥0.99, MSE, RMSE, max\|Δ\|, sum_ratio) remain pending real nanobrag_torch simulator. Artifacts: parity/ subdirectory with metrics.json, traces, diff heatmaps. Trace workflow: `docs/spec-db-tracing.md:10-26`. Collection log: `plans/active/PARITY-HARNESS-002/reports/2025-10-29T010131Z/collect_DB_AT_001.log` (3 tests). |
