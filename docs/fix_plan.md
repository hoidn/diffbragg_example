...
### 2025-12-01T230800Z - PERF-WARM-SIM-001 Phase D.4: Warm-cache detector retarget rebuild (READY FOR IMPLEMENTATION)
- **Evidence review**: Panel diagnostics (2025-12-01T223500Z) prove Stage C offsets collapse to ≤1.5e-08 mm yet chi² remains +0.067% above Stage A and the delta is uniform across every panel. Telemetry shows `delta_mm≈-0.25` (correct sign) but the warm-cache simulations never reflect the new geometry, implying `_retarget_stage_a_detectors` updates the `Detector` objects without rebuilding the cached `Simulator` instances (which still hold the original per-panel pixel grids). As soon as Stage C forces panel validations, the stale simulators keep emitting the pre-offset Bragg tensors, so masking/target stats match Stage A while chi² stagnates.
- **Plan**:
  1. Patch `dbex/refinement/stage_c_impl.py::_retarget_stage_a_detectors` so each panel rebuilds the nanobrag `Simulator` after applying the distance delta (and refresh any ROI-entry simulators) instead of only swapping the `.detector` attribute. Reuse Stage A's cached beam config + HKL grid to keep tensors hot, but ensure the new simulator binds the updated detector so subsequent warm-cache evaluations see the corrected distances.
  2. Keep the tensor-based distance deltas (GRADIENT-004) and Stage A context wiring intact; only the simulator rebuild path changes.
  3. Rerun `tests/dbex/test_torch_refine_smoke.py::test_stage_c_detector_microslip` for both `--smoke-detector-size=small` and `--smoke-detector-size=full`, capturing collect-only logs, pytest logs, telemetry JSONs, the warm-cache summarizer output, and refreshed panel diagnostics under `plans/active/PERF-WARM-SIM-001/reports/2025-12-01T230800Z/`.
  4. Compare panel-diagnostic deltas (Stage A vs Stage C) after the simulator rebuild to confirm χ² parity returns (≤0.05% regression) while detector-offset telemetry still reports ≥99.999% reduction.
- **Artifacts**: `plans/active/PERF-WARM-SIM-001/reports/2025-12-01T230800Z/` (collect logs, pytest logs, telemetry_stage_c_small/full.json, stage_c_warm_cache_report.json, panel_diag compare outputs, summary.md).

### 2025-12-01T235900Z - PERF-WARM-SIM-001 Phase D.4: Warm-cache simulator rebuild (READY FOR IMPLEMENTATION)
- **Context:** Panel diagnostics confirm Stage C detector offsets collapse yet chi² remains +0.067 % because `_retarget_stage_a_detectors` only refreshes Detector objects; the cached nanobrag `Simulator` instances (panel + ROI) still hold the pre-offset pixel grids so warm-cache evaluations ignore the new geometry.
- **Plan:**
  1. Update `dbex/refinement/stage_c_impl.py::_retarget_stage_a_detectors` so each affected panel rebuilds both the Detector *and* its cached Simulator after applying the distance delta, reusing `stage_a_ctx.beam_config`, device/dtype, and the existing crystal pointer (Stage C reattaches the Stage A final crystal immediately via `_retarget_stage_a_simulators`).
...
### 2025-12-02T083500Z - ARCH-STAGE-CONTEXT-001 Phase B.3.2: Stage B/C telemetry dataclasses (PLANNING)
- Stage B/C still mutate anonymous telemetry dicts inside `_build_stage_b_params`, `_run_stage_b_lbfgs`, `_build_stage_c_params`, and `_run_stage_c_lbfgs`, so the data-clump problem persists for the remaining stages and RefinementEngine/tests must keep poking string keys. After migrating Stage A to `StageATelemetryState`, the next loop introduces `StageBTelemetryState` and `StageCTelemetryState` in `dbex/refinement/context.py`, mirroring Stage A’s fields (loss/chi²/MSE traces, perf counters, variance-floor stats, best snapshots, baseline parity metadata, panel diagnostics) with typed defaults and docstrings referencing ARCH-STAGE-CTX-001 + PHYSICS-LOSS-001/002.
- Implementation plan:
  1. Extend `dbex/refinement/context.py` with the two new dataclasses plus helper shims so any legacy call site can detect dict vs dataclass. Include optional fields for REFINE-FLOW-001 baseline parity (`stage_b_baseline_rel_diff`, `stage_b_baseline_abs_diff`, `stage_b_baseline_diff_path`) and Stage C panel diagnostics (`panel_loss_diag_c`).
  2. Update Stage B code paths (`_build_stage_b_params`, `_check_stage_b_baseline_parity`, `_run_stage_b_lbfgs`, `StageB._build_lbfgs_closure`, `StageB.run`) to switch to attribute access while keeping compatibility branches for dict inputs until the CLI monolith is retired. Ensure StageBArtifacts + RefinementTelemetry still emit the existing schema.
  3. Update Stage C code paths (`_build_stage_c_params`, `StageC._build_lbfgs_closure`, `_run_stage_c_lbfgs`, `StageC.run`, panel-diagnostics writer) in the same style so telemetry mutation happens via attributes but `DBEX_STAGE_C_PANEL_DIAG_DIR` continues to write JSON.
- Validation (artifacts reserved at `plans/active/ARCH-STAGE-CONTEXT-001/reports/2025-12-02T083500Z/`):
  1. `AUTHORITATIVE_CMDS_DOC=./docs/TESTING_GUIDE.md DBEX_SMOKE_SIGMA_SOURCE=cli_override DBEX_SMOKE_DETECTOR_SIZE=small KMP_DUPLICATE_LIB_OK=TRUE NANOBRAGG_DISABLE_COMPILE=1 pytest -vv tests/dbex/test_torch_refine_smoke.py::test_stage_b_shell_modifiers --smoke-detector-size=small`
  2. `AUTHORITATIVE_CMDS_DOC=./docs/TESTING_GUIDE.md DBEX_SMOKE_SIGMA_SOURCE=cli_override DBEX_SMOKE_DETECTOR_SIZE=small KMP_DUPLICATE_LIB_OK=TRUE NANOBRAGG_DISABLE_COMPILE=1 pytest -vv tests/dbex/test_stage_b_per_reflection_smoke.py::test_stage_b_per_reflection_smoke --smoke-detector-size=small` (per-reflection failure signature still expected per TORCH-REFINE-004).
  3. `AUTHORITATIVE_CMDS_DOC=./docs/TESTING_GUIDE.md DBEX_SMOKE_SIGMA_SOURCE=cli_override DBEX_SMOKE_DETECTOR_SIZE=small KMP_DUPLICATE_LIB_OK=TRUE NANOBRAGG_DISABLE_COMPILE=1 pytest -vv tests/dbex/test_torch_refine_smoke.py::test_stage_c_detector_microslip --smoke-detector-size=small`
  4. Repeat the Stage C smoketest with `DBEX_SMOKE_DETECTOR_SIZE=full` (known PERF-WARM-SIM-001 χ² regression persists; capture logs, telemetry, and panel diagnostics under the report directory).
