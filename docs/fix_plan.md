...
- [ARCH-GRADIENT-FLOW-001] (Gradient Flow Restoration — DB-AT-010 Unblock) — **in_progress** (2025-12-08T120000Z: **Spec barrier removed via SPEC-INTERP-TRICUBIC-001** — spec now mandates tricubic interpolation for all stages, enabling cell parameter gradients through HKL lookup. Previous `interpolation=False` mandate for Stage A caused zero gradients by construction (nearest-neighbor uses `torch.round()` which has zero gradient). Remaining: implementation to wire `interpolation=True` in Stage A path. Artifacts: `plans/active/SPEC-INTERP-TRICUBIC-001/implementation.md`)
  - **Governed by:** GRADIENT-001, RUNTIME-001, TESTING-003
  - **Unblocked by:** SPEC-INTERP-TRICUBIC-001
- [SPEC-INTERP-TRICUBIC-001] (Global Tricubic Interpolation Default) — **in_progress** (2025-12-08T120000Z: Phase A spec changes complete. Specs updated: spec-db-core.md, spec-db-workflow.md, spec-db-conformance.md. Tricubic interpolation now canonical for ALL stages. Phase B (implementation) and Phase C (validation) pending. Root cause of cell gradient failure: spec mandated `interpolation=False` for Stage A, causing zero gradients through `torch.round()`. Fix: mandate tricubic globally to enable differentiable HKL lookup. Artifacts: `plans/active/SPEC-INTERP-TRICUBIC-001/implementation.md`)
