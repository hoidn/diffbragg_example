...
- [ARCH-GRADIENT-FLOW-001] (Gradient Flow Restoration — DB-AT-010 Unblock) — **blocked_pending_environment** (2025-12-08T000000Z i=141 LIFECYCLE DECISION: Implementation budget exhausted after 3 loops (i=138 evidence, i=139 Option A/B, i=140 Option C). Production code audit found 0 UNSAFE gradient-breaking patterns. Test harness fixes attempted via tensor-valued overrides (pre-creation and post-creation patterns), but detector Jacobian mismatch (~21,556× off) UNCHANGED across pattern variations. Beam wavelength confirmed BLOCKED by nanobrag_torch.simulator.py:761 external dependency (torch.tensor() detachment). Blocker classification: external_dependency (nanobrag_torch DetectorConfig/simulator gradient handling). RECOMMENDED PATH: Maintainer investigation with reproducer (detector Jacobian magnitude error + beam gradient detachment). ALTERNATIVE PATHS: (B) spec_change to relax tolerances/xfail DB-AT-010, (C) defer gradient-safe profile to future release. Artifacts: `plans/active/ARCH-GRADIENT-FLOW-001/reports/2025-12-08T000000Z/lifecycle_decision.md` (comprehensive evidence summary, hypothesis timeline, unblock paths). Previous loops: i=138 (call graph + suspect audit), i=139 (pre-creation overrides, partial gradient restoration), i=140 (post-creation overrides, hypothesis REJECTED).)
...
- [ARCH-SIM-CONSTRUCTION-001] (Simulator Construction Convention Alignment) — **blocked_pending_environment** (**Blocked:** C.34–C.39 evidence originally pointed to a missing `(Na·Nb·Nc)²` SQUARE lattice boost and was interpreted as a sincg lattice-factor bug in `nanobrag_torch`. The upstream `nanobrag_torch` maintainers have since responded (`inbox/nanobrag_torch_response_2025_12_08.md`), demonstrating that integrated/summed intensity SHOULD scale ∝ `Na·Nb·Nc` (peak height ∝ `(Na·Nb·Nc)²`), and that the observed ratios from our probes are consistent with correct physics rather than a simulator defect. Further plan-local instrumentation would violate PROBE-FREEZE-001; the initiative remains blocked pending (a) DBEX-side spec/test updates to adopt the correct SQUARE lattice scaling, (b) any Spec‑DB change that tightens or relaxes DB-AT-028/029 accordingly, or (c) a harness-grade diagnostic initiative outside plan-local probes. Artifacts: `plans/active/ARCH-SIM-CONSTRUCTION-001/reports/2026-01-13T150000Z/summary.md` (omega diagnosis correction) plus the maintainer response in `inbox/`.)
...
- [SPEC-SQUARE-PARTIALITY-001] (SQUARE Lattice Spec & Test Alignment) — **pending** (2025-12-08T000000Z: physics clarified by `inbox/nanobrag_torch_response_2025_12_08.md` — peak height ∝ `(Na·Nb·Nc)²`, integrated intensity ∝ `Na·Nb·Nc`. This initiative will align DBEX-side specs, findings, and partiality tests/probes with that behavior and then update ARCH-SIM-CONSTRUCTION-001’s ledger row accordingly. Working plan: `plans/active/SPEC-SQUARE-PARTIALITY-001/implementation.md`.)
...
- Status: **in_progress** (Phase A complete; Phase B unblocked — upstream nanobrag_torch fix landed per `inbox/from_nanobragg.md`)
- Type: architecture
