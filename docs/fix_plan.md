...
### 2025-12-02T094500Z - ARCH-STAGE-CONTEXT-001 Phase B.3.2: Stage B/C telemetry dataclasses (COMPLETE)
- Implemented `StageBTelemetryState` and `StageCTelemetryState` in `dbex/refinement/context.py` (lines 598-741) with PHYSICS-LOSS-001/002 + ARCH-STAGE-CTX-001 docstrings, and updated all Stage B/C helpers/wrappers (`dbex/refinement/stage_b_impl.py` lines 738-1020, `dbex/refinement/stage_b.py` lines 780-1040, `dbex/refinement/stage_c_impl.py` lines 330-700, `dbex/refinement/stage_c.py` lines 150-800) to construct/use the dataclasses while keeping temporary dict-compat shims. `_check_stage_b_baseline_parity` still emits REFINE-FLOW-001 telemetry, and the Stage C panel diagnostics hook (`DBEX_STAGE_C_PANEL_DIAG_DIR`) now stores JSON via `StageCTelemetryState.panel_loss_diag` instead of mutating anonymous dicts.
- Validation:
  1. Stage B shell smoketest (`tests/dbex/test_torch_refine_smoke.py::test_stage_b_shell_modifiers --smoke-detector-size=small`) — PASSED, telemetry `stage_b_baseline_rel_diff=8.7e-08` unchanged (`plans/active/ARCH-STAGE-CONTEXT-001/reports/2025-12-02T094500Z/pytest_stage_b_shell.log`).
  2. Stage C small smoketest (`...::test_stage_c_detector_microslip --smoke-detector-size=small`) — PASSED with unchanged chi² trace (log copied into the same report directory).
