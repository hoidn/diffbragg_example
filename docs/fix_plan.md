...
### 2025-12-01T121221Z - ARCH-REFINE-001 Phase B.2: JobContext scaffolding (READY FOR IMPLEMENTATION)
...
- **Implementation Checklist:**
  1. Extend `dbex/refinement/context.py` with a `JobContext` dataclass plus `build_job_context(...)` helper (docstring cites docs/spec-db-workflow.md §§32-41 and docs/config_crosswalk.md §3) capturing: CLI args namespace, DataLoad handle, calibration metadata payload (or None), sigma provenance + reference value, `RefinementConfig`, HKL metadata dict, ASU map, spot scale override, and optional extras (telemetry sinks). Validate tensor-bearing fields (HKL metadata halo flag, ASU map dtype).
  2. Update `dbex/refine_one.py::run_nanobrag_backend` to construct the `JobContext` once per invocation after sigma/calibration/HKL objects exist. Thread it through `run_nanobrag_refinement(...)` (new keyword) alongside existing arguments, stash the ASU map for later phases, and log provenance (sigma source, HKL source) into the context. Ensure the new helper lives behind Environment Freeze guardrails (stdlib only, no new deps).
  3. Update `dbex/nanobrag_refinement.py::run_nanobrag_refinement` to accept `job_context: Optional[JobContext]` (default None), include it in the `engine_inputs` dict (`'job_context': job_context`), and document that stage wrappers may pull it from `inputs['job_context']` for future telemetry/IO work. Adjust call sites/tests (e.g., `tests/dbex/test_refine_one_cli.py` mocks) so they assert the JobContext object flows through.
- **Validation:** Re-run the small-detector Stage smokes with telemetry per docs/TESTING_GUIDE.md §1.1 (`pytest -vv tests/dbex/test_torch_refine_smoke.py -k "test_stage_a_expansion or test_stage_b_shell_modifiers or test_stage_c_detector_microslip" --smoke-detector-size=small`) on the `sp.proc/refGeom_small` assets enumerated in docs/data_dependency_manifest.md:52-108, plus a focused CLI selector (e.g., `pytest -vv tests/dbex/test_refine_one_cli.py::test_nanobrag_backend_runs_simulator`) to prove JobContext wiring doesn’t break CLI guards. Set `AUTHORITATIVE_CMDS_DOC=./docs/TESTING_GUIDE.md DBEX_SMOKE_SIGMA_SOURCE=cli_override DBEX_SMOKE_DETECTOR_SIZE=small DBEX_SMOKE_TELEMETRY_PATH=<report>/telemetry_stage_smokes_small.json KMP_DUPLICATE_LIB_OK=TRUE NANOBRAGG_DISABLE_COMPILE=1`.
- **Artifacts:** plans/active/ARCH-REFINE-001/reports/2025-12-01T121221Z/ (collect_stage_smokes_small.log, pytest_stage_smokes_small.log, pytest_cli_job_context.log, telemetry_stage_a_small.json, telemetry_stage_b_small.json, telemetry_stage_c_small.json, summary.md)
