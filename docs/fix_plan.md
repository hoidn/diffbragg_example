...
- [ARCH-GRADIENT-FLOW-001] (Gradient Flow Restoration — DB-AT-010 Unblock) — **partial** (2025-12-08T230000Z i=171: Phase B.6 implemented single-line fix at `forward.py:196` — added `crystal_overrides=crystal_overrides` to `create_crystal_config` call. Fix eliminates "disconnected graph" error and enables gradient flow through crystal cell parameter path. NEW BLOCKER: Jacobian mismatch discovered — analytical gradient ~7e7, numerical gradient ~4.7e10 (≈640× difference with sign flip). Graph IS connected, but gradient magnitudes are wrong. Root cause hypothesis: gradient computation bug in `nanobrag_torch/models/crystal.py::compute_cell_tensors()` chain. **Next:** Escalate to nanobrag_torch gradient audit (new initiative ARCH-GRADIENT-MAGNITUDE-001 recommended). Artifacts: `plans/active/ARCH-GRADIENT-FLOW-001/reports/2025-12-08T230000Z/`)
