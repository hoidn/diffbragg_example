...
- [ARCH-GRADIENT-FLOW-001] (Gradient Flow Restoration — DB-AT-010 Unblock) — **unblocked** (2025-12-09T060000Z: **UPSTREAM FIX RECEIVED — Phase B.10 ready.** Response `inbox/mosaic-gradient-fix-response-2025-12-08.md`: mosaic gradient bug fixed in nanobrag_torch commit `1df032c2` via deterministic seeding + reparameterization trick. All 6 gradient tests pass in nanobrag_torch. **Next:** Pull latest nanobrag_torch, set `mosaic_seed` in CrystalConfig, re-run DB-AT-010 gradcheck. Artifacts: `plans/active/DB-AT-SUITE-CARE-001/reports/2025-12-09T060000Z/`)
  - **Governed by:** GRADIENT-001, GRADIENT-003, RUNTIME-001, TESTING-003
  - **ROOT CAUSE FIXED:** Mosaic code path used `torch.randn()` without seeded generator — now uses `torch.Generator` with deterministic seed + reparameterization trick (`actual_angles = frozen_noise * scale_param`)
  - **Upstream fix commit:** `1df032c2`
  - **Next:** Phase B.10 — Pull latest nanobrag_torch, verify DB-AT-010 passes with real mosaic parameters
