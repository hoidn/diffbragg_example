...
- [ARCH-GRADIENT-FLOW-001] (Gradient Flow Restoration — DB-AT-010 Unblock) — **blocked_pending_architecture** (2025-12-08T220000Z: **ROOT CAUSE IDENTIFIED.** Graph connectivity RESTORED (analytical gradients non-zero). Magnitude mismatch (1000×-76000×) caused by **discontinuous forward function**: HKL grid is only 34.7% occupied (65% zeros from missing reflections). Gradcheck requires continuous functions; tricubic interpolation over sparse grid creates discontinuities. Upstream tests pass with `default_F=100` (constant fill). **Architecture decision required** to resolve: (A) accept gradcheck failure for incomplete data, (B) test-only smooth mode, or (C) synthetic 100% coverage dataset. See SPEC-INTERP-TRICUBIC-001 for details. Artifacts: `plans/active/SPEC-INTERP-TRICUBIC-001/reports/2025-12-08T140000Z/`)
  - **Governed by:** GRADIENT-001, RUNTIME-001, TESTING-003
  - **Unblocked by:** SPEC-INTERP-TRICUBIC-001 (graph connectivity restored)
  - **Blocked by:** Architecture decision on gradient testing strategy for discontinuous forward functions
  - **Next:** User decision on approach A/B/C above
- [SPEC-INTERP-TRICUBIC-001] (Global Tricubic Interpolation Default) — **partial** (2025-12-08T140000Z: **Phase C validated: PARTIAL SUCCESS.** DB-AT-010 gradcheck 5/5 FAILED (magnitude mismatch) BUT analytical gradients ARE non-zero (graph connectivity RESTORED). Applied `halo=True` fix — still fails. **Root cause:** Grid occupancy is only 34.7% (65% zeros); discontinuities from F=0 fill for missing reflections break gradcheck assumptions. Upstream tests pass because they use `default_F=100` constant fill. **Architecture decision required:** (A) accept gradcheck failure as expected for incomplete data, (B) add test-only smooth mode with mean|F| fill, or (C) create 100% coverage synthetic test dataset. Phase A/B goals achieved; Phase C blocked_pending_architecture_decision. Artifacts: `plans/active/SPEC-INTERP-TRICUBIC-001/reports/2025-12-08T140000Z/`)
