...
- [PERF-WARM-SIM-001] (Warm Simulator) — **blocked — Stage C skips trusted-mask gating so panel-mode χ² drifts +0.067 %** (2025-12-01T210900Z: Full-detector telemetry shows `stage_a_final_chi2=2.10706464e+08` while every Stage C validation records `2.10848512e+08` even with zero detector offsets. Stage A’s full validations intersect `loss_mask` with the DIALS trusted mask inside `_compute_variance_weighted_loss` (`dbex/refinement/stage_a_impl.py:1336-1365`), but Stage C’s closure never applies `stage_a_ctx.trusted_masks_t` or `inputs.trusted_mask` before computing the variance-weighted loss (`dbex/refinement/stage_c_impl.py:512-593`). Until Stage C mirrors the trusted-mask gate in both ROI-mode closures and panel validations, its chi-squared baseline will remain inflated and REFINE-007 can’t pass even before optimization.)
