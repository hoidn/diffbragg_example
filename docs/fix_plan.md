...
- [ARCH-GRADIENT-FLOW-001] (Gradient Flow Restoration — DB-AT-010 Unblock) — **in_progress** (2025-12-07T212000Z: Phase A.1-A.2 complete (i=138). Evidence collection identified 0 UNSAFE patterns in production code; found 2 critical test harness gradient breaks (test_gradients.py:383 detector_distance, :496 beam_wavelength) using `.item()` to extract scalars for non-differentiable dxtbx geometry construction. Crystal tests (cell_a, cell_gamma) use correct tensor-preserving pattern but still fail; suspect external nanobrag_torch.models.Crystal constructor may break gradient. Next: Phase B.1 to fix detector/beam test harness (40-60 LOC) OR Phase A.3 probe to diagnose crystal constructor. Blocks: Gradient-Safe Profile conformance, DB-AT-SUITE-CARE-001 portfolio advancement. Artifacts: `plans/active/ARCH-GRADIENT-FLOW-001/reports/2025-12-07T212000Z/` (call_graph_trace.md, suspect_audit.md, 6 grep files, summary.md))
