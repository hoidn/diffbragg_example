...
- [ARCH-GRADIENT-FLOW-001] (Gradient Flow Restoration — DB-AT-010 Unblock) — **partial** (2025-12-08T050000Z i=153: Applied upstream nanobrag_torch fixes per inbox/from_nanobragg.md. Created `as_tensor_preserving_grad` utility in nanobrag_torch/utils/tensor_utils.py. Updated Simulator.__init__ to use gradient-preserving pattern for wavelength/fluence/kahn_factor. Converted Detector.distance/pixel_size/close_distance to properties enabling post-creation override pattern. Authored enforcement test `tests/architecture/test_gradient_contracts.py` (5 tests PASS). DB-AT-010 beam_wavelength now shows non-zero analytical gradient (-4.13e10), confirming partial gradient flow restoration. BLOCKED: DB-AT-010 still fails — crystal_overrides and detector_overrides in DBEX layer need separate work to thread gradients through simulate_forward_torch → create_unified_simulator → Crystal/Detector construction. **Next:** DBEX-layer gradient fixes (separate initiative scope). Artifacts: `plans/active/ARCH-GRADIENT-FLOW-001/reports/2025-12-08T050000Z/`)
...
- Status: **in_progress** (Phase A complete; Phase B.6 confirmed finite-detector root cause; Phase B.7 pending — test update to 400×400 detector)
- Type: spec+tests
- Priority: High
- Tier: 1
- Owner/Date: Galph ↔ Ralph / 2025-12-08
