...
- [ARCH-GRADIENT-FLOW-001] (Gradient Flow Restoration — DB-AT-010 Unblock) — **done** (2025-12-09T220000Z: **Phase B.10 COMPLETE.** All 6 DB-AT-010 gradcheck tests PASS. Upstream mosaic seed fix (`1df032c2`) verified available. DBEX integration issue identified: variance-weighted chi-squared loss uses IRLS with detached variance, which is correct physics but incompatible with `torch.autograd.gradcheck`. Fix: test fixtures now use MSE loss path (`sigma_readout=None`) for gradcheck validation. See GRADIENT-004 finding. Artifacts: `plans/active/ARCH-GRADIENT-FLOW-001/reports/2025-12-08T223000Z/gradcheck_mse_path.log`)
  - **Governed by:** GRADIENT-001, GRADIENT-003, GRADIENT-004, RUNTIME-001, TESTING-003
  - **ROOT CAUSE #1 (upstream):** Mosaic code path used `torch.randn()` without seeded generator — fixed in `1df032c2` via deterministic seeding + reparameterization trick
  - **ROOT CAUSE #2 (DBEX integration):** Variance-weighted chi-squared loss (`compute_masked_mse_loss` with sigma_readout) uses IRLS with detached variance denominator. This is correct physics for refinement but incompatible with gradcheck. Fix: use MSE loss path for gradient verification
  - **All 6 tests PASS:** cell_a, cell_gamma, cell_a_no_mosaic, detector_distance, beam_wavelength, combined wrapper
...
- [ARCH-SIM-CONSTRUCTION-001] (Simulator Construction Convention Alignment) — **done** (2025-12-09T060000Z: **No simulator bug exists.** Upstream maintainer analysis (`inbox/sincg_aggregation_bug_response_2025_12_09.md`) proves: (1) Test pixel at h≈100, k≈0.05, l≈-0.05 — only h is at Bragg peak, (2) sincg(π×0.05, 29) = -6.3 not 29, so F_latt = 41×(-6.3)×(-6.1) = 1,574 not 38,048, (3) Observed ~6,000× ratio is CORRECT for this geometry, (4) With 256×256 detector, peak ratios achieve 92.6% of expected consistently, (5) Proposed patches would break spec compliance. **Resolution:** Test geometry was flawed; no code bug. DB-AT-028/029 owned by DB-AT-SUITE-CARE-001. See `docs/findings.md::SIM-CONSTR-GEOMETRY-001` for lessons learned.)
...
- [ARCH-REFACTOR-001] (Refinement Engine Modularization & Physics Separation) — **done** (2025-12-09T070000Z i=250: Phase D.5 COMPLETE — facade deleted. Exit Criteria #1-4 satisfied. Artifacts: `plans/active/ARCH-REFACTOR-001/reports/2025-12-09T070000Z/`)
...
- [PHYSICS-LOSS-CONSISTENCY] (Physics Loss Function Alignment) — **done** (2025-12-08T230000Z i=251: **Reality check closure.** All exit criteria satisfied by prior completed work: (1) Stage A/B/C all use `_compute_variance_weighted_loss` from `dbex/physics/loss.py`, (2) telemetry tracks both chi_squared + masked_mse, (3) sigma-floor tests PASS (4/4 in `test_physics_loss_current.py`), (4) sigma-map/external_lookup tests PASS (8/8 in `test_data_load_sigma_map.py`). Total 12/12 tests PASS. No new implementation required. Artifacts: `plans/active/PHYSICS-LOSS-CONSISTENCY/reports/2025-12-08T230000Z/closure_summary.md`).
...
- [ARCH-STAGE-CONTEXT-CONSOLIDATION] (Stage Context Parameter Consolidation) — **in_progress** (2025-12-09T080000Z i=255: Phases A-D complete; Phase E (enforcement test) pending).
  ...
  - **Working Plan:** `plans/active/ARCH-STAGE-CONTEXT-CONSOLIDATION/implementation.md`
  - **Attempts History:**
    * 2025-12-09T010000Z (Loop i=253, Ralph) — **Phase C.1 complete.** Refactored `_build_stage_a_params` signature from 13 positional parameters to single typed context. Signature: `_build_stage_a_params(self, config: 'RefinementConfig', input_ctx: 'StageAInputContext')`. Function body unpacks context for backwards compatibility. Call site constructs `StageAInputContext` before calling. Tests: 6/6 context tests PASS, Stage A expansion collects OK. Artifacts: `plans/active/ARCH-STAGE-CONTEXT-CONSOLIDATION/reports/2025-12-09T010000Z/`.
    * 2025-12-09T070707Z (Loop i=254, Galph) — **Phase C.2 delegated.** Verified Phase C.1 complete. Delegating `_build_stage_b_params` refactoring (stage_b.py:93-111) from 16 positional params to `(self, config, input_ctx: StageBInputContext)`. Phase C.3 confirmed skip (Stage C already uses `RefinementSharedContext`). input.md written. Artifacts: `plans/active/ARCH-STAGE-CONTEXT-CONSOLIDATION/reports/2025-12-09T070707Z/`.
    * 2025-12-09T070707Z (Loop i=254, Ralph) — **Phase C.2 complete.** Refactored `_build_stage_b_params` signature from 16 positional parameters to `(self, config: 'RefinementConfig', input_ctx: 'StageBInputContext')`. Function body unpacks all 16 fields from context for backwards compatibility. Call site in `run()` constructs `StageBInputContext` before calling. Tests: 6/6 context tests PASS, 6/6 smoke tests collect OK. Metrics: No behavior changes. Artifacts: `plans/active/ARCH-STAGE-CONTEXT-CONSOLIDATION/reports/2025-12-09T070707Z/`. Next: C.4 (update remaining call sites) or Phase D (eliminate dict mutations).
    * 2025-12-09T080000Z (Loop i=255, Galph) — **Phases C-D verified complete.** Phase C.3 skip confirmed (Stage C already has clean signature); C.4 verified (one call site each, both updated). Phase D verified: grep for `telemetry['key'] = value` mutations found ZERO matches — all telemetry flows through typed dataclasses. Exit criteria 1-2 satisfied. Delegating Phase E (enforcement test). Artifacts: `plans/active/ARCH-STAGE-CONTEXT-CONSOLIDATION/reports/2025-12-09T080000Z/`.

### Tier 3: Performance & Memory
**Goal:** Profile and optimize GPU memory usage to enable full smoke tests on 24GB GPUs.
- [PERF-GPU-MEM-001] (GPU Memory Usage Analysis and Optimization) — **done** (2025-12-09T000000Z: **Phase C complete.** Threaded `pixel_batch_size` through DBEX refinement engine. Stage A smoke PASSED with `pixel_batch_size=32` on 24GB GPU. Physics unchanged — partiality tests pass. Exit criteria (3) and (4) met.)
  - **Governed by:** RUNTIME-001
  - **Depends on:** nanobrag_torch chunked interpolation fix ✓ RESOLVED
  - **Exit Criteria:** (1) Memory profiling report ✓; (2) ≥30% peak memory reduction (via chunking); (3) Stage A smoke completes on 24GB GPU ✓; (4) Physics unchanged (partiality/gradcheck tests pass) ✓
  - **Upstream feature:** `pixel_batch_size` CLI arg + `Simulator._run_chunked()` — **Recommended: 32 rows for 24GB GPU** (tricubic interpolation overhead requires smaller chunks than originally estimated)
  - **Attempts History:**
    ...
    * 2025-12-09T000000Z (Loop i=241, Ralph) — **Phase C complete.** Threaded `pixel_batch_size` through all refinement engine call sites (Stage A/B/C, reconstruction). Stage A smoke PASSED with `pixel_batch_size=32` (128 caused OOM during reconstruction due to tricubic interpolation overhead). Physics unchanged — partiality tests pass (2/2). Artifacts: `plans/active/PERF-GPU-MEM-001/reports/2025-12-09T000000Z/{summary.md, stage_a_pixel_batch_3.log, partiality_parity.log}`.
...
- Attempts History:
  ...
  * 2025-12-09T060000Z (Loop i=238, Ralph) — **UPSTREAM FIX RECEIVED — Phase B.10 ready**. Response `inbox/mosaic-gradient-fix-response-2025-12-08.md` confirms mosaic gradient bug fixed in nanobrag_torch commit `1df032c2`. Root cause: `torch.randn()` without seeded generator in `_generate_mosaic_rotations()`. Fix: deterministic `torch.Generator` + reparameterization trick (`actual_angles = frozen_noise * mosaic_spread_rad`). All 6 upstream gradient tests pass. **Integration steps:** (1) Pull latest nanobrag_torch, (2) Set `mosaic_seed` in CrystalConfig, (3) Re-run DB-AT-010. **Note:** Crystal cell gradient issue may have been coupled to mosaic bug — re-test with fix applied. Artifacts: `plans/active/DB-AT-SUITE-CARE-001/reports/2025-12-09T060000Z/`.
...
- Attempts History:
  ...
  * 2025-12-09T180000Z (Loop i=231, Ralph) — **Maintenance mode continues**: Verified inbox/outbox status — no new upstream responses. nanoBragg outbox last modified 2025-12-07 19:55; DBEX inbox last modified 2025-12-08 13:15. Two outstanding upstream requests remain: (1) `mosaic_gradient_bug_2025_12_08.md` (HIGH, blocks ARCH-GRADIENT-FLOW-001/DB-AT-010), (2) `chunked_interpolation_request_2025_12_09.md` (MEDIUM, blocks PERF-GPU-MEM-001). All Tier 0 initiatives remain blocked pending upstream. Artifacts: `plans/active/DB-AT-SUITE-CARE-001/reports/2025-12-09T000000Z/`.
  * 2025-12-09T004550Z (Loop i=232, Ralph) — **Maintenance mode continues**: Verified inbox/outbox status — no new upstream responses. nanoBragg outbox last modified 2025-12-07 19:55 (unchanged); DBEX inbox last modified 2025-12-08 13:15 (already processed). Two outstanding upstream requests remain: (1) `mosaic_gradient_bug_2025_12_08.md` (HIGH, filed Dec 8 14:35, blocks ARCH-GRADIENT-FLOW-001/DB-AT-010), (2) `chunked_interpolation_request_2025_12_09.md` (MEDIUM, filed Dec 8 14:51, blocks PERF-GPU-MEM-001). All Tier 0 initiatives remain blocked pending upstream. Artifacts: `plans/active/DB-AT-SUITE-CARE-001/reports/2025-12-09T004550Z/`.
  * 2025-12-09T004550Z (Loop i=233, Ralph) — **Maintenance mode continues**: Verified inbox/outbox status — no new upstream responses. nanoBragg outbox last modified 2025-12-07 19:55 (unchanged since last check); DBEX inbox last modified 2025-12-08 13:15 (no new files). Two outstanding upstream requests remain: (1) `mosaic_gradient_bug_2025_12_08.md` (HIGH, filed Dec 8, blocks ARCH-GRADIENT-FLOW-001/DB-AT-010), (2) `chunked_interpolation_request_2025_12_09.md` (MEDIUM, filed Dec 8, blocks PERF-GPU-MEM-001). All Tier 0 initiatives remain blocked pending upstream. Artifacts: `plans/active/DB-AT-SUITE-CARE-001/reports/2025-12-09T004550Z/`.
