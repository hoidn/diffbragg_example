...
### 2025-12-01T221500Z - PERF-WARM-SIM-001 Phase D.4: Stage A/C panel-loss parity (READY FOR IMPLEMENTATION)
- **Evidence review**: Repeat telemetry from 2025-12-01T214200Z shows Stage C detector offsets reduce to ≤1.5e-08 mm yet every Stage C full validation equals 210848512.0 while Stage A canonical trace ends at 210706464.0. All trusted-mask tensors are all-True, ROI closures remain aligned, and best-snapshot rehydration/log-scale/orientation fixes are already in place. The residual 0.0674% drift therefore stems from the only remaining divergence: Stage C re-implements Stage A’s panel-mode loss logic instead of reusing it, so subtle drift (mask ordering, dtype conversions, tensor stacking) keeps creeping in.
- **Plan**:
  1. **dbex/refinement/stage_a_impl.py** — Extract the panel-mode branch of `compute_loss` (lines 1450-1565) into a new helper (e.g., `_compute_panel_loss_from_stage_a_ctx`) that accepts the necessary context (StageAContext, detector, beam, crystal overrides, trusted masks, loss/target/sigma tensors, log-scale clamp, sigma floor tensor, panel_ids, `force_panel_eval`). The helper should encapsulate simulator reuse, trusted-mask intersection, tensor stacking, and variance-floor telemetry. Update Stage A’s `compute_loss` panel branch to call this helper to avoid behavior changes.
  2. **dbex/refinement/stage_c_impl.py::_build_stage_c_lbfgs_closure** — Replace the duplicated panel-mode logic (lines ~512-618) with a call to the new helper after retargeting detectors and building the Stage C crystal. Pass the same arguments Stage A would (StageAContext, detector, beam, crystal overrides, etc.) so Stage C measures the exact same pixel population before detector offsets are adjusted. Keep the ROI-mode branch unchanged.
  3. Ensure the helper and both callers preserve warm-cache semantics (reuse `stage_a_ctx.simulators` and `trusted_masks_t` when present, fall back to cold instantiation otherwise) and keep variance-floor telemetry identical.
  4. **Validation** — Rerun `tests/dbex/test_torch_refine_smoke.py::test_stage_c_detector_microslip` for both detector sizes capturing collect-only logs, pytest logs, telemetry, and the warm-cache summarizer under `plans/active/PERF-WARM-SIM-001/reports/2025-12-01T221500Z/`:
     ...
     - Repeat both commands for `--smoke-detector-size=full` saving artifacts to the same directory.
...
- **Success criteria**: Stage C initial/full chi² measurements now match Stage A final within the REFINE-007 tolerance (≤0.05% regression), all detector-offset gates remain green, and the summarizer report shows aligned `chi_squared_initial`, `chi_squared_final`, and ROI/perf counters for both detector sizes.
