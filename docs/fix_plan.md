...
- [ARCH-GRADIENT-FLOW-001] (Gradient Flow Restoration — DB-AT-010 Unblock) — **blocked_pending_environment** (2025-12-07T230000Z i=140: Phase B.1 Option C refactor complete but HYPOTHESIS REJECTED. Implemented post-creation override pattern symmetrical to crystal_overrides (-12 LOC: config_factories.py -21, forward.py +9). Test results: 0/2 PASS. Detector distance: SAME Jacobian mismatch signature as i=139 (numerical 2.39e12, analytical 1.11e8, ~21,556× off) — post-creation pattern did NOT resolve issue. Beam wavelength: BLOCKED by external dependency (nanobrag_torch.simulator.py:761 torch.tensor() detaches autograd graph, confirmed). Blocker classification: Both detector AND beam blocked_pending_environment (nanobrag_torch DetectorConfig distance handling OR simulator gradient chain issue). Hypothesis invalidated: Override mechanism pattern (pre vs post-creation) is NOT the root cause. Issue likely in nanobrag_torch DetectorConfig field assignment behavior or simulator distance derivative calculation. Next: Escalate to nanobrag_torch maintainer with reproducer (detector Jacobian magnitude error + beam gradient detachment). Artifacts: `plans/active/ARCH-GRADIENT-FLOW-001/reports/2025-12-07T230000Z/` (option_c_implementation_summary.md, pytest_detector_distance_option_c.log, pytest_beam_wavelength_option_c.log))
