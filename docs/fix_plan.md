...
- Depends on: PARITY-HARNESS-001, TORCH-BRIDGE-001, TORCH-CLI-003
- Status: in_progress
- Owner/Date: Unassigned / 2025-10-29
- Exit Criteria:
  1. Document the parity dataset gap and ship a manifest schema + checksum validation stub per `docs/parity_harness_spec.md:28-41`, ensuring helpers fail closed until an official dataset is published.
  2. Author reusable parity harness utilities (metrics, trace capture, artifact writers) aligned with `docs/parity_harness_spec.md:78-199` and tracing workflow in `docs/spec-db-tracing.md:10-26`.
  3. Implement pytest selector `DB_AT_001` enforcing correlation ≥ 0.99 and RMSE thresholds (`docs/spec-db-conformance.md:24-26`) with metrics/trace artifacts stored under `plans/active/PARITY-HARNESS-002/reports/<timestamp>/parity/`.
  4. Update testing registries (`docs/TESTING_GUIDE.md` §2, `docs/development/TEST_SUITE_INDEX.md`) to reflect the new Active selector(s) with `pytest --collect-only` logs archived; ledger Attempts History captures Metrics/Artifacts lines for each run.
- Working Plan: plans/active/PARITY-HARNESS-002/implementation.md
- Attempts History:
  * 2025-10-29T010009Z — Supervisor planning kickoff; validated parity harness spec requirements, confirmed DB_AT_001/002 selectors remain Planned with 0 collection, and authored implementation plan phases A-E to cover dataset mirroring, harness utilities, parity/determinism tests, and doc sync. Metrics: pending. Artifacts: pending.
  * 2025-10-29T010131Z — Completed Phase A (A1-A4) parity harness bootstrap: Probed for nanoBragg2 golden data (not found); generated FALLBACK dataset under tests/fixtures/golden_data/simple_cubic/ with scripts/generate_simple_cubic_golden.py (synthetic Bragg, refGeom targets, manifest.json with SHA256 checksums); implemented tests/fixtures/parity_loader.py with checksum validation, [panel, slow, fast] ordering guards, and pixel pitch validation per docs/spec-db-core.md:24,43; authored tests/dbex/test_db_at_001_parity.py with 3 manifest integrity tests (checksum validation, tensor shapes, pixel pitch guards); all tests pass; re-ran smoke baseline (3/3 passed); updated docs/TESTING_GUIDE.md §2 and docs/development/TEST_SUITE_INDEX.md to mark DB_AT_001 Active (partial) with collection evidence. Metrics: 3/3 DB_AT_001 tests passed, 3/3 smoke tests passed, 1.07s + 1.82s runtime, CPU, Python 3.9.23, pytest 8.4.2. DB_AT_001 collection: 3 tests (up from 0). Manifest SHA256: df88c7d20757f0900c2829d01ceb00fade96b68203bfbd00e4fcb1b5238ebf6d. Artifacts: plans/active/PARITY-HARNESS-002/reports/2025-10-29T010131Z/{notes_phase_a.md,generate_golden_data.log,checksums.txt,pytest_DB_AT_001.log,collect_DB_AT_001.log,pytest_smoke.log,collect_smoke.log,summary.md}. First Divergence: n/a (manifest-only tests). Next Actions: Phase B harness utilities (compute_parity_metrics, artifact writers, trace capture); Phase C full parity test (correlation thresholds, diff heatmaps); blocked pending real nanobrag_torch simulator.
