...
- [ARCH-GRADIENT-FLOW-001] (Gradient Flow Restoration — DB-AT-010 Unblock) — **blocked** (2025-12-07T220000Z i=139: Phase B.1 implementation complete but BLOCKED. Implemented detector_overrides/beam_overrides (85 LOC: forward.py +35, config_factories.py +10, test_gradients.py -40 net). Test results: 0/2 PASS. Detector distance: gradient flow restored (analytical grad 1.14e8 non-zero) but Jacobian mismatch (numerical 5.24e11, ~4590× off). Beam wavelength: BLOCKED by external dependency — nanobrag_torch.simulator.py:761 calls torch.tensor(wavelength_A) which detaches autograd graph (confirmed via pytest warning). Blocker classification: beam=blocked_pending_environment (external nanobrag_torch), detector=implementation issue (magnitude error). Next: Option C (refactor to post-creation override pattern matching crystal_overrides, 1 loop) OR Option B (escalate nanobrag_torch blocker with reproducer). Deferred: full DB-AT-010 suite (crystal tests not run). Artifacts: `plans/active/ARCH-GRADIENT-FLOW-001/reports/2025-12-07T220000Z/` (phase_b1_analysis.md, pytest_detector_distance_post_fix.log, pytest_beam_wavelength_post_fix.log, summary.md))
