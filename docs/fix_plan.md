...
- [ARCH-GRADIENT-FLOW-001] (Gradient Flow Restoration — DB-AT-010 Unblock) — **in_progress** (2025-12-08T130000Z: **Spec barrier removed and implementation wired via SPEC-INTERP-TRICUBIC-001 Phase B** — config default now `enable_hkl_interpolation=True`, enabling cell parameter gradients through tricubic HKL lookup. **Next:** Run DB-AT-010 gradcheck to verify cell parameter gradients now flow through the updated default. Artifacts: `plans/active/SPEC-INTERP-TRICUBIC-001/reports/2025-12-08T130000Z/`)
  - **Governed by:** GRADIENT-001, RUNTIME-001, TESTING-003
  - **Unblocked by:** SPEC-INTERP-TRICUBIC-001
- [SPEC-INTERP-TRICUBIC-001] (Global Tricubic Interpolation Default) — **in_progress** (2025-12-08T130000Z: **Phase B complete.** Config default changed: `enable_hkl_interpolation=True` in `config.py:42`. Test file updated with legacy comments (3 locations in `test_stage_a_smoke_parity.py`). Validation: Partiality tests 2/2 PASS; Stage A smoke shows tricubic working (99.79% HKL hit rate); OOM during reconstruction is env constraint, not code regression. **Phase C (validation)** pending: Run DB-AT-010 gradcheck to verify cell parameter gradients now flow. Artifacts: `plans/active/SPEC-INTERP-TRICUBIC-001/reports/2025-12-08T130000Z/`)
