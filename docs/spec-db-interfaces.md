...
- Detector readout noise (`sigma_readout`) SHALL be resolved using the canonical ladder in `spec-db-core.md` (§Variance inputs). Any valid sigma map (from config or CLI) beats any scalar (from config or CLI); any map beats external tiles; any scalar beats external tiles. Conflicts between sources at the same tier (e.g., config vs CLI map or scalar) SHALL be treated as configuration errors and MUST cause the CLI to exit non‑zero. When none of map/scalar/external tiles are available, the CLI SHALL error (no silent defaults such as legacy ~3 ADU fallbacks).
