...
- Sigma sourcing (Spec‑DB conformance): sigma_readout MUST follow the canonical ladder in `spec-db-core.md` (map → scalar → external tiles → error; conflicts at the same tier are errors). Shapes must match the target or the run fails. Legacy hardcoded defaults (e.g., ~3 ADU) are non‑conformant. The general calibration ladder (config → CLI → external_lookup → MTZ defaults → hardcoded) SHALL NOT be used to invent sigma sources.
...
- Purpose: Provide a fast-running refinement harness that exercises the Stage A/B/C code paths without exhausting GPU VRAM. For DB‑AT selectors and mapping‑aligned Stage‑A visuals, the canonical baseline is the DB‑AT‑024 golden simple_cubic mapping configuration (`tests/fixtures/golden_data/simple_cubic/refined.{expt,refl,structure_factors.mtz}`, `config_torch.json`, `747_mask.pkl`) wired through the mapping helpers/fixtures (e.g. `refgeom_dataload`, `build_mapping_stage_a_context`). The `refGeom`/`refGeom_small` smoke fixtures under `sp.proc/` are perf/diagnostic only and SHALL NOT be treated as canonical for DB‑AT‑024/027/028/029.
