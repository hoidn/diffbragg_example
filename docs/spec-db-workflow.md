...
- Precedence ladder (highest → lowest): CLI overrides > external_lookup payloads > refined MTZ metadata > raw MTZ defaults > hardcoded defaults. When the same field (gain/adu_per_photon, sigma_readout, sigma_floor, spot_scale_override, beam flux/exposure, beamsize_mm, N_cells) appears at multiple tiers, implementations SHALL pick the highest priority and emit a warning + provenance if lower tiers disagree. Refined MTZ, when requested, SHALL be consumed or the run SHALL fail fast (no silent fallback to raw).
