Summary: Capture canonical DiffBragg + nanobrag_torch forward tensors for DB-AT-001 and stage parity evidence for manifest replacement.
Mode: Parity
...
Artifacts: plans/active/NANOBRAG-GOLDEN-001/reports/2025-10-29T063817Z/{golden_dataset/legacy/bragg_diffbragg.npy,golden_dataset/legacy/config_diffbragg.json,golden_dataset/torch/bragg_torch.npy,golden_dataset/torch/config_torch.json,golden_dataset/torch/panel_metrics.json,golden_dataset/metrics.json,golden_dataset/roi_metrics.csv,golden_dataset/logs/canonical_capture.log,collect_db_at_001_parity.log,collect_db_at_001_forward.log}
Do Now:
  1. NANOBRAG-GOLDEN-001::A2 (plans/active/NANOBRAG-GOLDEN-001/implementation.md) — Capture the GPU DiffBragg baseline (`bragg_diffbragg.npy`, `config_diffbragg.json`) plus canonical capture log under 2025-10-29T063817Z/golden_dataset/legacy/, using devId=0 to avoid the DIFFBRAGG-001 CPU bug. tests: none.
  2. NANOBRAG-GOLDEN-001::A3 (plans/active/NANOBRAG-GOLDEN-001/implementation.md) — Run the nanobrag_torch forward simulator across all panels to emit `bragg_torch.npy`, per-panel target/loss mask snapshots, `config_torch.json`, and `panel_metrics.json`, aligning configs with CONFIG-001 mappings. tests: none.
  3. NANOBRAG-GOLDEN-001::B2 (plans/active/NANOBRAG-GOLDEN-001/implementation.md) — Re-collect DB_AT_001 parity and forward selectors with the staged artifacts, archiving logs in 2025-10-29T063817Z/ to satisfy TESTING-003 before manifest rewrites. tests: KMP_DUPLICATE_LIB_OK=TRUE pytest --collect-only -q tests/dbex/test_db_at_001_parity.py -k DB_AT_001; KMP_DUPLICATE_LIB_OK=TRUE pytest --collect-only -q tests/dbex/test_forward_equivalence_complete.py -k DB_AT_001.
Priorities & Rationale:
- docs/forward_equivalence.md:21-33 — Canonical capture must produce paired DiffBragg and torch tensors plus ROI metrics before parity deltas are interpreted.
- docs/spec-db-conformance.md:23-26 — DB_AT_001 conformance hinges on archived forward baselines and documented acceptance thresholds.
- docs/config_crosswalk.md:15-33 — Detector, beam, and crystal configs must respect CUSTOM conventions and mask polarity when hydrating nanobrag_torch.
- docs/nanobrag_api.md:23-44 — Torch simulator expects swapped beam-center ordering, sample→source vectors, and per-panel masks, informing the capture script.
- docs/TESTING_GUIDE.md:85-86 — Selector documentation remains accurate only if new collect-only logs accompany the canonical artifact swap.
- docs/architecture.md:32 — The bridge module is the integration seam; canonical dataset capture validates the architecture contract.
How-To Map:
- `mkdir -p plans/active/NANOBRAG-GOLDEN-001/reports/2025-10-29T063817Z/golden_dataset/{legacy,torch,logs}`
- `cat <<'PY' > plans/active/NANOBRAG-GOLDEN-001/reports/2025-10-29T063817Z/capture_forward.py`
```python
#!/usr/bin/env python3
import datetime
import json
...
```
PY
- `CUDA_VISIBLE_DEVICES=0 KMP_DUPLICATE_LIB_OK=TRUE python3 -u plans/active/NANOBRAG-GOLDEN-001/reports/2025-10-29T063817Z/capture_forward.py |& tee plans/active/NANOBRAG-GOLDEN-001/reports/2025-10-29T063817Z/golden_dataset/logs/canonical_capture.log`
- `KMP_DUPLICATE_LIB_OK=TRUE pytest --collect-only -q tests/dbex/test_db_at_001_parity.py -k DB_AT_001 |& tee plans/active/NANOBRAG-GOLDEN-001/reports/2025-10-29T063817Z/collect_db_at_001_parity.log`
- `KMP_DUPLICATE_LIB_OK=TRUE pytest --collect-only -q tests/dbex/test_forward_equivalence_complete.py -k DB_AT_001 |& tee plans/active/NANOBRAG-GOLDEN-001/reports/2025-10-29T063817Z/collect_db_at_001_forward.log`
Pitfalls To Avoid:
- DevId must remain 0; CPU fallback (devId=-1) hits the diffBraggCUDA.cu:708 cleanup bug (DIFFBRAGG-001).
- Keep mask polarity True=include; converting to float32 tensors before handing to torch prevents inverted masks.
- Ensure structure-factor grid uses |F| magnitudes and metadata; missing padding skews tricubic interpolation.
- Avoid overwriting fallback fixtures under `tests/fixtures` until manifest update steps are explicit.
- Capture stdout/stderr via `tee` so diagnostics for both pipelines land in the report directory (DIAGNOSTICS-001).
- Confirm CUDA availability before running the torch simulator; abort and log if `torch.cuda.is_available()` is false.
- Do not leave `_temp.mtz` artifacts in repo root; cleanup lives inside the capture script.
- Respect Environment Freeze — no pip/mamba actions even if imports fail; log blockers instead.
- Keep ROI metrics sampling deterministic (seed=42) so parity comparisons remain reproducible (PARITY-001).
- Validate panel ordering `[panel, slow, fast]` whenever writing NumPy arrays to avoid spec-db-core mismatches.
If Blocked: Capture the failing command output under `plans/active/NANOBRAG-GOLDEN-001/reports/2025-10-29T063817Z/golden_dataset/logs/`, append blocker notes plus return conditions to docs/fix_plan.md Attempts History, update planning_notes, and log `state=blocked` with `next_action=switch_focus` in galph_memory before pivoting.
Findings Applied (Mandatory):
- CONFIG-001 — Torch config hydration obeys beam-center swaps, square-pixel guards, and sample→source vectors.
- CONFORMANCE-001 — DB_AT_001 acceptance thresholds drive the need for archived forward baselines and diagnostics.
- DIFFBRAGG-001 — Forces GPU execution for DiffBragg export; CPU mode remains broken at diffBraggCUDA.cu:708.
- DIAGNOSTICS-001 — Canonical capture logs keep torch diagnostics artifact trail intact.
- TESTING-003 — Selector status stays authoritative only if fresh collect-only logs accompany dataset updates.
- PARITY-001 — ROI sampling and first-divergence readiness rely on deterministic tensor ordering and mask alignment.
Doc Sync Plan (Mandatory):
- `KMP_DUPLICATE_LIB_OK=TRUE pytest --collect-only -q tests/dbex/test_db_at_001_parity.py -k DB_AT_001 |& tee plans/active/NANOBRAG-GOLDEN-001/reports/2025-10-29T063817Z/collect_db_at_001_parity.log`
- `KMP_DUPLICATE_LIB_OK=TRUE pytest --collect-only -q tests/dbex/test_forward_equivalence_complete.py -k DB_AT_001 |& tee plans/active/NANOBRAG-GOLDEN-001/reports/2025-10-29T063817Z/collect_db_at_001_forward.log`
- Update docs/TESTING_GUIDE.md §2 and docs/development/TEST_SUITE_INDEX.md entries to reference the 2025-10-29T063817Z logs once captured and ensure selector status remains Active with >0 collected tests.
