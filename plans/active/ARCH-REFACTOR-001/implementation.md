...
- [x] D.2: **CLI Refactor** — ✓ COMPLETE (2025-12-02T220000Z, commit 46389946) Migrated `dbex/refine_one.py::run_nanobrag_backend()` from `run_nanobrag_refinement` facade to direct RefinementEngine instantiation following 5-step Engine pattern. Updated imports (removed facade, added build_refinement_context, RefinementEngine, StageA/B/C). Built RefinementContext after JobContext with baseline_detector conditional on enable_stage_c. Instantiated RefinementEngine with conditional stage list (A always, B/C based on CLI flags). Replaced facade call with engine.run({"context": ...}) and extracted artifacts from engine._artifacts. Preserved downstream logic unchanged (telemetry, MSE, logging, exception handling). Tests: 2/2 CLI selectors PASSED (test_torch_diagnostics_metadata 2 parametrizations 0.95s, test_nanobrag_backend_runs_simulator 1.25s). HDF5 /torch_diagnostics attributes unchanged (backward compatible). Telemetry structure preserved. Metrics: +19 lines net (Engine instantiation + context build + artifact extraction). CLI now uses RefinementEngine directly per ARCH-REFACTOR-001 Exit Criterion #2.
  - **Validation:** ✓ `pytest -vv tests/dbex/test_refine_one_cli.py::test_torch_diagnostics_metadata tests/dbex/test_refine_one_cli.py::test_nanobrag_backend_runs_simulator` (2/2 tests PASSED)
