...
#### C.36 — Subpixel offset telemetry (Planned — 2026-01-08T150000Z loop)
- [ ] **Goal:** Capture the actual subpixel offset grids for the slow/fast axes under `_compute_physics_for_position` so we can determine whether both axes truly straddle Δ≈0 after Phase C.33 or if one axis still samples only positive deltas.
- [ ] **Instrumentation scope:**
  1. When `collect_partiality_stats` **and** `trace_pixel` are enabled, record the per-axis subpixel offsets (fast/slow, optionally phi/mosaic) or their HKL deltas before sincg evaluation under `_partiality_stats['subpixel_offsets']`.
...
- [ ] **Validation:** Rerun the single-pixel probe (oversample=13, N_cells=41/29/32) capturing the new telemetry and rerun `pytest -vv tests/architecture/test_nanobrag_partiality.py::test_square_lattice_applies_ncells --maxfail=1` to ensure instrumentation is benign. Evidence will decide whether the next implementation loop patches the offset construction or escalates to spec_change/increased oversample.
