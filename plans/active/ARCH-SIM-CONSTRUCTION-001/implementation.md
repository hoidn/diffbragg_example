...
#### C.35 — Oversample normalization fix for SQUARE lattice (Implementation-ready — 2026-01-08 loop)
- [ ] **Diagnosis recap:** Coverage metrics under `reports/2026-01-05T150000Z/` and HKL tensor telemetry under `reports/2026-01-07T150000Z/` show SQUARE lattices rarely sample Δ≈0 along k/l, so dividing by `steps = sources·phi_steps·mosaic_domains·oversample²` dilutes the few contributing subpixels by 169×. Treating SQUARE lattices as Riemann sums (omit oversample²) keeps the lattice boost intact without touching other lattice shapes.
- [ ] **Implementation plan (Environment-Freeze exception):**
  1. Edit `/home/ollie/Documents/diffbragg_example/src/nanobrag-torch/src/nanobrag_torch/simulator.py::Simulator.run` so the `steps` scalar skips the `oversample * oversample` factor when `crystal.shape == CrystalShape.SQUARE`, and expose the chosen scalar via `_partiality_stats['steps_scalar']` for instrumentation.
  2. Save the diff as `plans/active/ARCH-SIM-CONSTRUCTION-001/patches/square_lattice_steps_fix.patch`, rebuild/install nanobrag_torch (`python -m pip install -e src/nanobrag-torch --no-deps`), and tag the environment (`nanobrag-partiality-2026-01-08`) in `patches/environment_tag.md`.
  3. Update `docs/findings.md::SIM-CONSTR-PARTIALITY-001` with the normalization rationale. (Tests already exist; no additional pytest nodes required beyond the mapped selectors.)
  4. Validation: rerun the single-pixel probe + coverage summary, `pytest -vv tests/architecture/test_nanobrag_partiality.py::test_square_lattice_applies_ncells`, and DB-AT-028/029 (`pytest -vv tests/dbex/test_stage_a_smoke_parity.py -k \"DB_AT_028 or DB_AT_029\"`) with artifacts rooted at `plans/active/ARCH-SIM-CONSTRUCTION-001/reports/2026-01-08T010000Z/`. Expect the observed ratio to match `(Na·Nb·Nc)²` within ≤1% and DB-AT chi²/ROI gates to drop sharply toward spec now that reconstruction/Stage A share simulator magnitude.
- [ ] **Exit criteria:** `(Na·Nb·Nc)²` parity is restored for both the single-pixel probe and the architecture enforcement test, DB-AT-028/029 trend toward spec, and Environment-Freeze bookkeeping (patch file, rebuild log, findings entry) is complete. If ratios remain ≤95% after the normalization fix, escalate to spec_change or revisit lattice kernel math.
