...
- [ ] **Align build_final_bragg_from_stage_a_telemetry cold path with Stage A telemetry:** When `stage_a_ctx` is unavailable for `param_state="initial"`, use the recorded Stage A masked-mean telemetry (or the mapping `masked_mean_ratio` metadata) to rescale the cold-path simulator output so the reconstructed `bragg_before` matches `telemetry.model_mean_masked`. This eliminates the 1.46× gap captured in `plans/active/ARCH-SIM-CONSTRUCTION-001/reports/2025-12-17T180000Z/stage_a_baseline_probe_baseline.json` and keeps DB-AT fixtures honest even when warm caches are pruned.
- [ ] **Harden tests and probes:** Extend `tests/dbex/test_artifact_parity.py` (new case: Stage A artifacts intentionally dropped before reconstruction) so it asserts the cold path reproduces the cached bragg stack’s masked mean within ≤1e-6 relative error. Update `plans/active/ARCH-SIM-CONSTRUCTION-001/bin/compare_stage_a_baseline.py` to log the correction factor and distinguish cache hits vs cold-path realignments in the JSON summary.
- [ ] **Validation:** Re-run the baseline probe in both geometry modes plus DB-AT-028/029 with artifacts under `plans/active/ARCH-SIM-CONSTRUCTION-001/reports/2025-12-18T010000Z/`. Success criteria: When the helper falls back to the cold path, the masked-mean ratio between reconstruction and telemetry is ≈1.0 (≤1e-3 drift), and DB-AT selectors record whether the cache was used so we can focus next on the remaining ROI correlation failure.
