...
+ [ ] **Mask provenance instrumentation:** Extend Stage A telemetry and the reconstruction diagnostics so we can confirm the masked-mean calculations are using the same loss mask that DB-AT gates enforce. Tasks:
    - In `dbex/refinement/stage_a.py`, record the mask cardinality (per-panel counts and overall pixel count) and a lightweight checksum whenever `target_mean_masked` / `model_mean_masked` are computed. Persist this metadata on the telemetry object so downstream consumers know exactly which mask the masked means refer to.
    - Update `dbex/refinement/reconstruction.py::build_final_bragg_from_stage_a_telemetry` so `baseline_stats.json` includes both the Stage A telemetry mask metadata and the reconstruction mask metadata (counts + checksum) next to the existing masked/unmasked means. This lets us see whether the 30× discrepancy is due to different masks or a genuine scale issue.
    - Teach `plans/active/ARCH-SIM-CONSTRUCTION-001/bin/compare_stage_a_baseline.py` to emit the new mask metadata in its JSON + console summary, and to flag mismatches explicitly.
    - After instrumentation, rerun the Stage A baseline probe and DB-AT-028/029 with fresh artifact directories so we have side-by-side evidence of mask parity.
- [ ] **Telemetry instrumentation follow-up:** Update the probe to read masked-mean telemetry from the top-level Stage A fields (instead of the `log_scale_effective` dict) and extend `dbex/refinement/reconstruction.py::build_final_bragg_from_stage_a_telemetry` with masked-mean diagnostics that log the reconstructed `bragg_before` vs telemetry deltas and persist them to a `baseline_stats.json` artifact next to the mask-coverage log.
