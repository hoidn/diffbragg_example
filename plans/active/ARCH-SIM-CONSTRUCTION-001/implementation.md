...
#### C.7 — Log-scale scale-factor telemetry + reconstruction sanity (Planned / In Flight)
...
- [ ] **Always compute masked target/model means when warm cache is available:** Move the Stage A masked-mean computation outside the `calibration_adjusted_for_n_cells` guard so `target_mean_masked` / `model_mean_masked` are populated on every calibrated run (ROI or panel) and emitted via telemetry.
- [ ] **Apply the measured ratio to `log_scale_baseline`:** When both masked means are positive, add `np.log(target_mean_masked / model_mean_masked)` to the existing baseline (or use it as the baseline when calibration metadata was absent) so zero-iteration Stage A predictions align with the observed masked intensity before any LBFGS delta is applied. Update `log_scale_baseline_source` and `StageAContext.log_scale_baseline` so downstream stages consume the corrected baseline.
- [ ] **Propagate the updated baseline through config/telemetry:** After adjusting the baseline, refresh `config.log_scale_baseline` and ensure the Stage A telemetry block (`param_deltas['log_scale_baseline']`) reports the new value so reconstruction helpers and DB-AT fixtures rebuild `bragg_before` with the aligned scale.
- [ ] **Validation:** Rerun `probe_stage_a_scale_alignment.py` to capture the new baseline plus `pytest -vv tests/dbex/test_stage_a_smoke_parity.py -k "DB_AT_028 or DB_AT_029"` (with artifact dirs) to prove chi²/ROI gates now pass with the corrected baseline before proceeding to any further reconstruction work.
