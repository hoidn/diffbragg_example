...
**Physics clarification (nanobrag_torch response — 2025-12-08):** The C.25–C.28 probes and SIM-CONSTR-PARTIALITY-001 originally assumed that **integrated/summed** SQUARE lattice intensity over a reflection should scale as `(Na·Nb·Nc)²`. The `nanobrag_torch` maintainers have since demonstrated (see `inbox/nanobrag_torch_response_2025_12_08.md`) that this expectation is incorrect: peak height at exact Bragg scales as `(Na·Nb·Nc)²`, but the **integrated intensity** scales linearly with `Na·Nb·Nc`. Our single-pixel, oversample=1 probes are therefore measuring peak behavior, while the oversample>1 probes are closer to the integrated regime. The “missing `(Na·Nb·Nc)²` boost” we chased in C.27/C.28 reflects this mismatch of expectations rather than a simulator defect. Future loops MUST treat the SQUARE partiality work in C.27/C.39 as historical context only and, if revived, should focus on updating DBEX-side specs/tests (e.g. `tests/architecture/test_nanobrag_partiality.py::test_square_lattice_applies_ncells`) to assert the correct linear scaling instead of editing `nanobrag_torch` to force `(Na·Nb·Nc)²` behavior for integrated intensity.
