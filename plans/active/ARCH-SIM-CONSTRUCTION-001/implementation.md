...
#### C.34 — Subpixel contribution audit (Planning — 2026-01-05 loop)
- [ ] **Instrument per-subpixel payloads:** Extend the existing partiality hook so that, when `collect_partiality_stats` and `trace_pixel` are both enabled, the simulator caches the per-subpixel `F_latt_a/b/c`, `delta_{h,k,l}`, and `F_total_squared_pre_lorentz` tensors for the traced pixel instead of only their global means. This keeps instrumentation inside the owner module (no new plan-local probes) while giving us decision-carrying coverage data.
- [ ] **Probe analysis:** Update `plans/active/ARCH-SIM-CONSTRUCTION-001/bin/probe_square_lattice_scaling.py` to summarize the traced pixel’s per-subpixel payload: count how many samples satisfy `|Δ_{k,l}| < 1/N` and compare the cumulative `F_total_squared_pre_lorentz` of those samples to the `(Na·Nb·Nc)^2` target. Emit a markdown+JSON block under `reports/2026-01-05T010000Z/` showing whether the intensity deficit tracks the fraction of subpixels that reach the central sincg lobe.
- [ ] **Decision gate:** If the audit proves only ~1/oversample² samples contribute, the next implementation loop will focus on correcting the accumulation/normalization step (e.g., weighting subpixels by their sincg coverage instead of uniform averaging). If coverage is healthy but ratios still collapse, pivot to inspecting the `steps` normalization and Lorentz/polar application order.

#### C.35 — Oversample normalization fix for SQUARE lattice (Planned — 2026-01-06 loop)
- [ ] **Diagnosis:** Coverage metrics under `plans/active/ARCH-SIM-CONSTRUCTION-001/reports/2026-01-05T150000Z/` show 0/169 subpixels hit `|Δ_{k,l}| < 1/N` for the canonical single-pixel probe (Na=41, Nb=29, Nc=32, oversample=13). Because `Simulator.run` always divides the summed intensity by `steps = sources·phi_steps·mosaic_domains·oversample²`, the one or two subpixels that carry the `(Na·Nb·Nc)²` lattice weight are diluted by 169× before scaling. NanoBragg’s reference implementation only works when the sincg ridge is densely sampled, which is not the case for Stage-A smoke geometry, so the SQUARE path needs shape-aware normalization.
- [ ] **Implementation plan:**
  1. Edit `/home/ollie/Documents/diffbragg_example_2/diffbragg_example/src/nanobrag-torch/src/nanobrag_torch/simulator.py::Simulator.run` so the `steps` scalar includes the `oversample * oversample` factor **only** when the crystal shape is not `CrystalShape.SQUARE`. For SQUARE lattices, set `oversample_norm = 1` so summed subpixel contributions represent the integral rather than the mean. Surface the selected normalization via `_partiality_stats['steps_scalar']` whenever the debug hook is enabled so probes/tests can assert the value.
  2. Update `tests/architecture/test_nanobrag_partiality.py::test_square_lattice_applies_ncells` to assert `(Na·Nb·Nc)²` parity with the new normalization and to fail fast if `steps_scalar` still contains `oversample²` in SQUARE mode. Keep the existing cpu/cuda coverage; add a regression guard that inspects the stored stats.
  3. Environment Freeze compliance: capture the diff as `plans/active/ARCH-SIM-CONSTRUCTION-001/patches/square_lattice_steps_fix.patch`, record the rebuild/tag commands in `plans/active/ARCH-SIM-CONSTRUCTION-001/patches/environment_tag.md`, and append a SIM-CONSTR-PARTIALITY-001 note in `docs/findings.md` describing the normalization change.
  4. Validation: rerun `plans/active/ARCH-SIM-CONSTRUCTION-001/bin/probe_square_lattice_scaling.py` (single-pixel probe + coverage summary), `pytest -vv tests/architecture/test_nanobrag_partiality.py::test_square_lattice_applies_ncells`, and DB-AT-028/029 (`pytest -vv tests/dbex/test_stage_a_smoke_parity.py -k "DB_AT_028 or DB_AT_029"`) with artifacts rooted at `plans/active/ARCH-SIM-CONSTRUCTION-001/reports/2026-01-06T010000Z/`. Expect the observed ratio to match `(Na·Nb·Nc)²` within ≤1% and the DB-AT chi²/ROI gates to drop sharply toward spec.
- [ ] **Exit criteria:** `(Na·Nb·Nc)²` parity is restored for both the single-pixel probe and the architecture enforcement test, and DB-AT-028/029 regressions move toward closure without new instrumentation. If ratios remain ≤95% of spec after the normalization fix, escalate to spec_change or revisit the lattice kernel math.
