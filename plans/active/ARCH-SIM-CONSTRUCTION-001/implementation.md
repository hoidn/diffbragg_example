...
#### C.13 — Mapping baseline scaling parity (Planned — 2025-12-17T180000Z)
- [ ] **Mirror Stage A masked-mean adjustment in mapping:** Update `dbex/vis/mapping.py::build_mapping_stage_a_context` so, after `simulate_forward_once` returns, the helper computes masked means for the target and `bragg_zero_iter`, scales the Bragg stack by their ratio (when both are positive/finite), and records the ratio in diagnostics. This keeps `bragg_zero_iter` on the same calibrated scale as the Stage A warm-cache baseline introduced in Phase C.10.
- [ ] **Telemetry alignment:** When the adjustment fires, tag `diagnostics["log_scale_baseline_source"]` appropriately (e.g., `"masked_intensity_ratio"`) and propagate the ratio into `calibration` metadata so Stage A can detect that mapping already performed the correction. Reset `inputs.global_scale_hint` to 1.0 after scaling to avoid double-applying the ratio during Stage A warm-starts.
- [ ] **Validation:** Re-run `compare_stage_a_baseline.py` (baseline + perturbed modes) and DB-AT-028/029 with artifacts under `plans/active/ARCH-SIM-CONSTRUCTION-001/reports/2025-12-17T180000Z/`. Success criteria: Stage A vs mapping max|Δ| < 1 ADU and RMSE near float precision in baseline mode; DB-AT selectors capture the updated telemetry before we move on to the Stage A vs target shape discrepancy.
