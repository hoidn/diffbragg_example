...
- [x] D3: Extend `StageBArtifacts` with optional `bragg_full` (respecting CPU fallback contexts), have `StageB.run` compute final Bragg when Stage C is disabled, and update the Stage A→B engine branch + writer plumbing to use the artifact instead of recomputing from telemetry. Re-run Stage A expansion + Stage B shell/per-reflection smokes (per-reflection failure signature expected) and the CLI writer test to prove telemetry/artifact parity stays intact.
    - [x] 2025-12-02T141500Z — StageB.run reconstruction payload + device mismatch fixed; Stage A gate relaxed; Stage B shell + Stage A expansion smokes PASS under artifact path (logs under `reports/2025-12-02T141500Z/`).
    - [x] D3.1 ✅ 2025-12-02T150500Z — All artifact assertions added; Stage A, Stage B shell, and CLI writer tests PASSED (logs under `reports/2025-12-02T150500Z/`). Original: D3.1 — Add explicit artifact assertions to `test_stage_a_expansion`, `test_stage_b_shell_modifiers`, and `test_stage_b_per_reflection_smoke` so they verify `StageAArtifacts.bragg_full`/`StageBArtifacts.bragg_full` whenever Stage B/C are disabled. Capture `engine_artifacts` even in the per-reflection test so the invariant is exercised once TORCH-REFINE-004 resumes.
    - [x] D3.2 ✅ 2025-12-02T150500Z — All tests validated; Phase D complete. Original: D3.2 — Re-run `tests/dbex/test_torch_refine_smoke.py::test_stage_a_expansion` and `::test_stage_b_shell_modifiers` (small detector, CLI sigma override) and archive logs in the next report directory. Re-run `tests/dbex/test_refine_one_cli.py::test_torch_diagnostics_metadata` to prove writer + artifact map stay in sync. Stage B per-reflection remains a known failure; capture the failure signature separately under PERF-WARM-SIM-001 when revisited.
