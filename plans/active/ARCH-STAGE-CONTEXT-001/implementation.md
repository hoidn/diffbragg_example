...
- [ ] A4: Apply the same pattern to Stage C detector retarget helpers, then update Stage A/B/C smoketests + `test_stage_a_engine_delegation_telemetry` expectations to assert that typed contexts propagate through the engine (context marker present in telemetry) before starting the Stage artifact work. *(2025-12-02T022454Z plan: extend `RefinementSharedContext` with an optional `baseline_detector`, build and thread the shared context through `stage_c.py`, add compatibility shims to `_build_stage_c_params` / `_build_stage_c_lbfgs_closure`, and rerun `test_stage_c_detector_microslip` for both detector sizes (collect + test + telemetry) under `plans/active/ARCH-STAGE-CONTEXT-001/reports/2025-12-02T022454Z/` before touching Stage artifacts.)*
