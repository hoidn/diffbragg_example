...
- [ ] B3: Replace mutable telemetry dicts with dataclasses that expose typed appenders plus `.to_refinement_telemetry()` adapters, ensuring schema parity.
  ...
  - [ ] **B3.2 — Stage B/C telemetry wrappers:** now replicate the pattern for Stage B (`telemetry_state` dict feeding shell/per-reflection diagnostics) and Stage C (distance-offset telemetry). Keep compatibility shims so archived tooling that still expects dict semantics can function until downstream initiatives flip fully to dataclasses. Validations MUST include Stage B shell + per-reflection smokes (per-reflection remains expected failure) and Stage C small/full smokes (full run continues to document the PERF-WARM-SIM-001 regression).
