...
- [x] C2: Surface collector-generated StageResult dataclasses through the CLI/writer path so typed telemetry survives the engine hand-off. Deliverables:
  * Added a `stage_result` attachment point to `RefinementTelemetry` (skipped in `to_dict()`), finalized Stage A/B/C collectors, and preserved the StageResult on each telemetry object.
  * Threaded `{label: stage_result}` maps through `run_nanobrag_refinement` → `run_nanobrag_backend` and added a `stage_results` kwarg (default `None`) to `dbex/io/writer.write_torch_outputs`, updating CLI tests/mocks accordingly. Artifacts/tests: `plans/active/ARCH-TELEMETRY-001/reports/2025-12-04T020000Z/` (`test_torch_diagnostics_metadata`, `test_stage_b_cpu_fallback.py::test_stage_b_baseline_guard_diff_payload`, `test_torch_refine_smoke.py::test_stage_b_shell_modifiers`). NOTE: Stage C microslip selector still needs to be rerun under the writer changes; capture it with the next Do Now when the writer serialization is updated.
- [ ] C3: StageResult-first writer serialization and shim retirement.
  * C3.1 — Teach `dbex/io/writer.py` to consume typed StageResult telemetry/perf counters when `stage_results` is provided, keeping `RefinementTelemetry` only for static metadata (optimizer, ROI counts, param_deltas). Fall back to the legacy dict path when stage results are unavailable (mocks/tests) and preserve `/torch_diagnostics` schema byte-for-byte.
  * C3.2 — Remove the remaining `legacy_telemetry_dict` plumbing in Stage A/B/C once the writer path relies on StageResult (drop `StageResult.to_legacy_dict()` usages, stop duplicating traces inside `RefinementTelemetry`, and adjust stage/unit tests accordingly).
