...
- [ ] A1: Reality-check canonical assets (`refGeom.expt`, `refGeom.refl`, `scaled.mtz`, `747_mask.pkl`) and capture inventory log under reports.
- [ ] A2: Reproduce calibration context by instantiating `DataLoad` with canonical args; record background-subtracted ROI sample metrics (mean, std, ROI sums) to anchor acceptance thresholds.
- [ ] A3: Cross-reference normative sources (`docs/spec-db-workflow.md §4`, `docs/architecture.md §13`, `docs/config_crosswalk.md`) and summarize calibration expectations (photon conversion vs ADU + global scale) inside report summary.
