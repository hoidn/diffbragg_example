...
### Checklist
- [ ] B1: Update `dbex.refine_one.run_nanobrag_backend` to instantiate nanobrag_torch simulators per panel, reusing post-sim scaling √(spot_scale_override).
- [ ] B2: Stream simulator outputs through `_write_torch_outputs`, ensuring artifacts (ROI dumps, metrics) mirror DiffBragg expectations and remain deterministic.
- [ ] B3: Add regression tests (e.g., `tests/dbex/test_nanobrag_backend.py`) comparing simulator output against canonical fixtures for at least one panel/ROI.
