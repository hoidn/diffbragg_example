...
def build_structure_factor_grid(indices, amplitudes, device):
    ...
    for (h, k, l), amp in zip(hkls, amps):
        # Patch: Convert numpy scalar to float before assigning to torch tensor
        grid[h - h_min, k - k_min, l - l_min] = float(amp)
    metadata = {
        "h_min": int(h_min),
        "h_max": int(h_max),
        "k_min": int(k_min),
...
def main():
    ...
    for cycle in range(num_macro):
        ...
        # Patch: Convert Famps back to flex.double for customized_copy
        # Root cause: Famps is a numpy array, but customized_copy expects flex arrays
