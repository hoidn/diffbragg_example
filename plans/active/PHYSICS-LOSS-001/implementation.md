...
- [ ] G4: Emit a tracked manifest (`sp.proc/sigma_metadata_manifest.json`) plus README coverage for `idx-0000_sigma_metadata.{expt,sigma_tiles.pkl}` (and the provenance JSON) by extending `plans/active/PHYSICS-LOSS-001/bin/embed_sigma_external_lookup.py` with a `--manifest` flag that records generator command, SHA256 hashes, and sigma stats so CI can detect fixture drift before metadata smokes run.
- [ ] G5: Add an automated validation selector (pytest module under `tests/sp_proc/test_sigma_metadata_fixture.py`) that reads the manifest, recomputes hashes, and loads the metadata experiment via `_load_external_lookup_sigma_map` to assert `sigma_readout_provenance="external_lookup"` as a CI gate for Stage A/B/C metadata runs.
