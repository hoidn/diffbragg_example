...
- [x] G1: Author `plans/active/PHYSICS-LOSS-001/bin/embed_sigma_external_lookup.py` that copies an `.expt` file, injects calibrated `ExternalLookupItemDouble` tiles (from `--sigma-map` or constant `--sigma-value`) for each panel, and documents provenance in a JSON/README so smoke tests can generate metadata fixtures deterministically.
- [x] G2: Extend smoke fixtures (`tests/conftest.py`) and `tests/dbex/test_torch_refine_smoke.py` to parameterize the sigma source (`cli_override` vs `metadata`), route metadata-backed datasets into `prepare_refinement_inputs`, and assert Stage A telemetry reports `sigma_readout_provenance="external_lookup"` when metadata is selected.
- [x] G3: Update `docs/TESTING_GUIDE.md` and `docs/development/TEST_SUITE_INDEX.md` with the metadata-fixture workflow (script invocation, required env vars, mapped selectors) and capture Stage A full-detector logs plus telemetry JSON showing the metadata provenance under `plans/active/PHYSICS-LOSS-001/reports/<timestamp>/`.
- [x] G4: Emit a tracked manifest (`sp.proc/sigma_metadata_manifest.json`) plus README coverage for `idx-0000_sigma_metadata.{expt,sigma_tiles.pkl}` (and the provenance JSON) by extending `plans/active/PHYSICS-LOSS-001/bin/embed_sigma_external_lookup.py` with a `--manifest` flag that records generator command, SHA256 hashes, and sigma stats so CI can detect fixture drift before metadata smokes run. *(Delivered 2025-11-21T083500Z — manifest + README snapshot archived in `plans/active/PHYSICS-LOSS-001/reports/2025-11-21T083500Z/{sigma_metadata_manifest.json,sigma_metadata.json}`.)*
- [x] G5: Add an automated validation selector (pytest module under `tests/sp_proc/test_sigma_metadata_fixture.py`) that reads the manifest, recomputes hashes, and loads the metadata experiment via `_load_external_lookup_sigma_map` to assert `sigma_readout_provenance="external_lookup"` as a CI gate for Stage A/B/C metadata runs. *(Delivered 2025-11-21T083500Z — `pytest -vv tests/sp_proc/test_sigma_metadata_fixture.py` artifacts stored alongside the manifest snapshot.)*

## Phase H — Stage B/C metadata smoke coverage
- [x] H1: Update Stage B smoke plumbing (`tests/conftest.py::smoke_dataset_paths`, `tests/dbex/test_torch_refine_smoke.py::test_stage_b_shell_modifiers`) so metadata sigma fixtures run without CLI overrides, asserting telemetry provenance/variance-floor stats for both sigma sources and maintaining REFINE-008 gates. *(Delivered 2025-11-21T071912Z — Stage B selector now honors `--smoke-sigma-source` with telemetry recorded in `plans/active/PHYSICS-LOSS-001/reports/2025-11-21T071912Z/pytest_stage_b_metadata.log`.)*
- [x] H2: Extend Stage C detector microslip smoke (`tests/dbex/test_torch_refine_smoke.py::test_stage_c_detector_microslip`) to honor the metadata sigma selector, confirming Stage C inherits the Stage A canonical chi-squared snapshot and still meets REFINE-007 offset-reduction gates. *(Delivered 2025-11-21T071912Z — Stage C metadata run asserts provenance + REFINE-007 gates; see `pytest_stage_c_metadata.log`.)*
- [x] H3: Refresh DB-AT/selector docs (`docs/TESTING_GUIDE.md`, `docs/development/TEST_SUITE_INDEX.md`, and the PHYSICS-LOSS-001 ledger) plus rerun Stage B/C full-detector smokes (and dependent DB-AT selectors) under `DBEX_SMOKE_SIGMA_SOURCE=metadata`, capturing pytest + telemetry artifacts for the report directory. *(Delivered 2025-11-21T071912Z — docs updated with metadata workflow + artifact pointers.)*

## Phase I — Metadata propagation to DB-AT + CLI diagnostics
- [x] I1: Parameterize `tests/dbex/test_mapping_consistency.py::TestDB_AT_024_Mapping::test_db_at_024_mapping_smoke` (and supporting fixtures) with `smoke_sigma_source` so metadata-backed datasets run without CLI overrides, assert `sigma_readout_provenance="external_lookup"` within mapping diagnostics, and log metadata telemetry/clamp stats in `mapping_metrics.json`. *(Delivered 2025-11-21T075449Z — see `plans/active/PHYSICS-LOSS-001/reports/2025-11-21T075449Z/mapping_metrics.json`.)*
- [x] I2: Extend `tests/dbex/test_refine_one_cli.py::test_torch_diagnostics_metadata` (and related CLI sigma tests) to cover `sigma_readout_provenance="external_lookup"` by injecting metadata-style sigma maps into the mocked `DataLoad`, verifying `/torch_diagnostics` attrs plus Stage A telemetry toggle correctly between CLI scalar and metadata sources. *(Delivered 2025-11-21T075449Z — `pytest_cli_metadata.log` documents both provenance branches.)*
- [x] I3: Update `docs/TESTING_GUIDE.md` §1.4 and `docs/development/TEST_SUITE_INDEX.md` to document the DB-AT-024 metadata workflow (env vars, artifact naming, pytest commands) and capture fresh CLI + DB-AT selector logs under `plans/active/PHYSICS-LOSS-001/reports/2025-11-21T075449Z/` demonstrating metadata provenance end-to-end. *(Delivered 2025-11-21T075449Z — docs cite DB-AT metadata guard + CLI telemetry artifacts.)*
