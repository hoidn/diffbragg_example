...
- [ ] H1: Update Stage B smoke plumbing (`tests/conftest.py::smoke_dataset_paths`, `tests/dbex/test_torch_refine_smoke.py::test_stage_b_shell_modifiers`) so metadata sigma fixtures run without CLI overrides, asserting telemetry provenance/variance-floor stats for both sigma sources and maintaining REFINE-008 gates.
- [ ] H2: Extend Stage C detector microslip smoke (`tests/dbex/test_torch_refine_smoke.py::test_stage_c_detector_microslip`) to honor the metadata sigma selector, confirming Stage C inherits the Stage A canonical chi-squared snapshot and still meets REFINE-007 offset-reduction gates.
- [ ] H3: Refresh DB-AT/selector docs (`docs/TESTING_GUIDE.md`, `docs/development/TEST_SUITE_INDEX.md`, and the PHYSICS-LOSS-001 ledger) plus rerun Stage B/C full-detector smokes (and dependent DB-AT selectors) under `DBEX_SMOKE_SIGMA_SOURCE=metadata`, capturing pytest + telemetry artifacts for the report directory.
