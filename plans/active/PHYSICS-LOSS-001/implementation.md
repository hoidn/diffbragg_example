...
- [ ] G4: Update the fixture manifests/checksums (e.g., `scripts/generate_simple_cubic_golden.py` manifest + new README) to track the embedded sigma maps and add CI verification (hash or checksum guard) so regenerated `.expt/.refl` files are detected when they drift.
- [ ] G5: Add an automated validation step (shell script or pytest mini-selector) that confirms the metadata-embedded refGeom assets load and expose `sigma_readout_provenance="external_lookup"`; wire it into CI so Phase G artifacts remain authoritative.
...
- [ ] I1: Parameterize `tests/dbex/test_mapping_consistency.py::TestDB_AT_024_Mapping::test_db_at_024_mapping_smoke` (and supporting fixtures) with `smoke_sigma_source` so metadata-backed datasets run without CLI overrides, assert `sigma_readout_provenance="external_lookup"` within mapping diagnostics, and log metadata telemetry/clamp stats in `mapping_metrics.json`.
- [ ] I2: Extend `tests/dbex/test_refine_one_cli.py::test_torch_diagnostics_metadata` (and related CLI sigma tests) to cover `sigma_readout_provenance="external_lookup"` by injecting metadata-style sigma maps into the mocked `DataLoad`, verifying `/torch_diagnostics` attrs plus Stage A telemetry toggle correctly between CLI scalar and metadata sources.
- [ ] I3: Update `docs/TESTING_GUIDE.md` §1.4 and `docs/development/TEST_SUITE_INDEX.md` to document the DB-AT-024 metadata workflow (env vars, artifact naming, pytest commands) and capture fresh CLI + DB-AT selector logs under `plans/active/PHYSICS-LOSS-001/reports/<timestamp>/` demonstrating metadata provenance end-to-end.
