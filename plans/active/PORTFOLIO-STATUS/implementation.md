...
### Phase D — Roll-Up Coverage Awareness (NEW)
...
- **D1 — Script & API updates:** Extend `plan_inventory.py` to record roll-up membership per plan (e.g., `rollup_coverage` field in `inventory.json`), treat those plans as tracked (`bucket=tracked_via_rollup`), exclude them from `inventory_missing.md`, and print the roll-up covered count in the console summary so future runs immediately show the split.
- **D2 — Tests:** Expand `plans/active/PORTFOLIO-STATUS/tests/test_plan_inventory.py` with hermetic coverage tests for the new helper that applies roll-up membership and ensures bucket/markdown outputs skip roll-up-covered plans. Cover both single-roll-up and multi-roll-up membership scenarios.
- **D3 — Ledger sync:** After the script/test changes land, rerun the inventory with the guard flag (target timestamp ≥ 2025-12-07T150000Z), capture artifacts, and update `docs/fix_plan.md` Plan Directory Inventory appendix counts. Expected outcome: `active_missing` drops close to zero (only genuinely uncovered plans remain), leaving `missing_plan = 1` (ARCH-REFRACTOR-001 stub) and `tracked + tracked_via_rollup` covering the rest.
