...
- **Phase A — Reality check & guard design**
  - [ ] A1: Reproduce failure (`pytest -v tests/dbex/test_refine_one_cli.py::test_torch_diagnostics_metadata`) to confirm TypeError signature; archive log.
  - [ ] A2: Inspect `_write_torch_outputs` to map score aggregation points and determine coercion strategy (float casting, fallback for NaN, empty ROI guard).
  - [ ] A3: Draft patch approach + test assertion updates; record in report summary.

- **Phase B — Implementation & tests**
  - [ ] B1: Update `_write_torch_outputs` to coerce ROI scores, guard empty collection, and emit consistent diagnostics (including prints).
  - [ ] B2: Extend `test_torch_diagnostics_metadata` to assert absence of TypeError and verify new aggregation behavior (e.g., fraction well-modeled equals expected value for mocked score).
  - [ ] B3: Run targeted pytest + collect-only; capture logs under `plans/active/TORCH-CLI-004/reports/<timestamp>/`.

- **Phase C — Documentation & ledger sync**
  - [ ] C1: Update `docs/fix_plan.md` Attempts History with metrics/artifacts; mark initiative status appropriately.
  - [ ] C2: Confirm `docs/TESTING_GUIDE.md` / `docs/development/TEST_SUITE_INDEX.md` entries remain accurate (note improved diagnostics guard if needed); update if behavior description changes.
  - [ ] C3: Append summary to `galph_memory.md` and archive new artifacts.
