...
- [x] B1: **Deep Diagnostic & Fix** — Escalated from LBFGS test failure (chi²=1.425B at step 0 despite bugfix 826f4c9). **COMPLETED (2025-11-22T180000Z):** Code audit (grep search for B_ideal sources) revealed root cause: dbex/nanobrag_refinement.py:786 was DISCARDING B_ideal from derive_u_matrix_from_mosflm_a_star and recomputing from cctbx cell (lines 801-818), causing U @ B_ideal_cctbx ≠ A*_MOSFLM. Fix: Use returned B_ideal_reciprocal_np directly (line 787), delete cctbx recomputation (removed 13 lines). Zero-point validation PASSED chi²=989k (unchanged). LBFGS test B1 revalidation RUNNING (expected 30-40 min, validation pending). See `code_audit_b_ideal_sources.md`, `b_ideal_fix_implementation.md`, `phase_b_deep_diagnostic_decision.md`.
