...
- [ ] C5: **Code Path Equivalence Diagnostic** — **PENDING**: Instrument `_stage_a_forward` to log A* checksums for both code paths (`use_mapping_zero_geometry=True` vs `False`), run 2-step diagnostic, and compare to identify exact divergence point. Target: Prove/disprove that round-trip conversion (MOSFLM A* → U, B_ideal → A* reconstruction → crystal_overrides) introduces numerical error causing catastrophic chi². If confirmed, implement fix to bypass crystal_overrides when all parameter deltas are zero OR fix numerical precision in U/B_ideal derivation.
- [ ] C6: **Phase 5 Convergence Test (D_full or U+scale)** — SKIPPED per C2 FAIL (chi²>8M, CC<0.9). Will not run D_full until alternative fix identified.
