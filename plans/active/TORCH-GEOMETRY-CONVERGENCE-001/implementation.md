...
- [ ] C6: **Code Path Divergence Fix Implementation** — **PENDING**: Implement Option 1 fix (bypass crystal_overrides at mapping zero point). Add zero-check logic in `_stage_a_forward`: when all parameter deltas are zero (q_params == q_initial, log_cell_X_delta == 0 for X in {a,b,c,alpha,beta,gamma}), bypass U/B_ideal round-trip and use direct MOSFLM A* injection path (`crystal_overrides=None`) even when `use_mapping_zero_geometry=False`. Validate fix with rerun of Phase C5 diagnostic (success criteria: delta_chi² < 1%, zero-point and first closure paths produce identical chi²). Regression guard test_stage_a_expansion MUST PASS. If fix succeeds: proceed to full Phase 5 convergence test (A_scale_only 10 steps, success criteria: CC ≥ 0.99, chi² drift ≤ 1%). If fix fails: escalate to Priority 2 (audit create_crystal_config internals for numerical precision bug in crystal_overrides handling).
- [x] C7: **Regression Guard** — PASSED (2025-11-22T235959Z): Regression guard test_stage_a_expansion passed, confirming LR config additions and script CLI flag do not break cell+misset default path. See `pytest_regression.log`.
- [ ] C8: **Findings Update** — Create CONVERGENCE-002 (or extend REFINE-001/GRADIENT-001) in `docs/findings.md` documenting: (a) root cause from Phase A, (b) fix choice from Phase B, (c) convergence metrics from Phase C, (d) usage conventions for quaternion U-matrix refinement.
- [ ] C9: **Fix-Plan Close** — Mark TORCH-GEOMETRY-CONVERGENCE-001 as `done`; unblock TORCH-GEOMETRY-PARITY-003 (if fix succeeds) or document quaternion approach as non-viable and recommend alternative (hybrid cell+quaternion+scale or other).
