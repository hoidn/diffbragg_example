...
## Purpose
Rebuild the structure-factor grid (or generate a derivative dataset) that remains valid when applying the deterministic Stage A perturbation (cell stretch + orientation misset) so the Stage A masked-MSE gate can run without `xfail`. This retires the temporary guard introduced in TORCH-REFINE-002, validates orientation telemetry against a meaningful perturbation, and clears findings REFINE-004/005 while documenting the revised ≥0.2% improvement floor supported by canonical assets.
...
2) Stage A smoke test re-enables the masked-MSE improvement assertion (no `xfail`), demonstrating ≥0.2% ROI improvement within ≤30 LBFGS iterations while asserting the deterministic misset telemetry.
3) REFINE-004 and REFINE-005 are updated (closed or downgraded) with artifact references; `docs/fix_plan.md` Attempts History captures the grid rebuild and probe results, and Stage A selector logs plus probe JSON are archived under `plans/active/TORCH-REFINE-002D/reports/<timestamp>/`.
...
- Phase 1 — HKL Grid Halo & Interpolation Enablement
  - [x] P1.1: Add optional ±1 halo support to `dbex.nanobrag_bridge.build_structure_factor_grid` so tricubic interpolation can sample fractional HKL coordinates without triggering `default_F`.
