...
## Purpose
Enable the Stage C refinement stage for the nanobrag torch backend by introducing per-panel detector translations along the panel normal. The goal is to recover deterministic detector distance miscalibrations (microslip) using LBFGS with the same closure/telemetry contract as Stage A, achieving ≥5% masked-MSE loss reduction within ≤30 steps and capturing per-panel parameter deltas for diagnostics.
...
2. Deterministic Stage C smoke (refGeom with synthetic panel offsets) achieves ≥5% masked-MSE improvement within ≤30 LBFGS iterations while maintaining non-increasing full-loss validations.
...
- Phase 0 — Baseline Reality Check
  - [ ] P0.1: Quantify masked-MSE impact of deterministic detector distance offsets (e.g., +0.25 mm along odet_vec) to confirm ≥5% headroom.
...
- Phase 4 — Validation & Ledger Updates
  - [ ] P4.1: Author `test_stage_c_detector_microslip` covering deterministic detector offsets, enabling Stage C, asserting ≥5% improvement and telemetry completeness.
...
- `tests/dbex/test_torch_refine_smoke.py::test_stage_c_detector_microslip` — new Stage C acceptance (≥5% improvement, per-panel deltas)
