...
ATTN NEW PROBLEMS:
- [x] **Stale plan directory inventory and archival** — Resolved via [PORTFOLIO-STATUS] (docs/fix_plan.md Tier 0, line 27). All Phases A–F complete (2025-12-08T190000Z): delivered inventory automation with roll-up-aware tracking, achieved 100% ledger coverage (55 total plans, 28 tracked directly, 34 via rollups, 0 active_missing), and archived stale directories (ARCH-LAZY-IMPORTS-001, ARCH-TELEMETRY-001, ARCH-BRIDGE-RESP-001) to archive/plans/ with full ledger reference updates. See `plans/active/PORTFOLIO-STATUS/reports/2025-12-07T220000Z/` for final inventory evidence.
---
...
IMPORTANT:NEW
- [x] **Freeze plan-local probe scripts in favor of parallel logging** — Plan-local diagnostic scripts under `plans/active/**/bin` have accumulated partial implementations of mapping/simulator/ROI/scale semantics, making them fragile "shadow pipelines" that diverge from production code. Directive: stop extending existing probe scripts and treat them as read-only evidence; new visibility must come from parallel logging/telemetry added inside real call paths (Stage A, reconstruction helpers, simulator factory) and consumed by small comparison tools/tests (e.g., architecture enforcement tests under `tests/architecture/`). Galph should schedule a harness/architecture initiative to (a) catalog existing probes, (b) deprecate or migrate any that encode physics into proper owner APIs + tests, and (c) update prompts/docs so future loops default to parallel logging rather than new probes. **Tracking:** Added Tier‑0 initiative [ARCH-PROBE-FREEZE-001] with implementation plan at `plans/active/ARCH-PROBE-FREEZE-001/implementation.md` (docs/fix_plan.md updated 2025-12-26T230000Z). **RESOLVED (2026-01-02T180000Z):** All phases complete. Enforcement test `tests/architecture/test_probe_contracts.py` + governance docs (`prompts/supervisor.md::diagnostic_script_policy`, `docs/findings.md::PROBE-FREEZE-001`) now enforce probe freeze policy mechanically. See `docs/fix_plan.md` Tier 0 + `plans/active/ARCH-PROBE-FREEZE-001/reports/2026-01-02T180000Z/`.
IMPORTANT:NEW
- [ ] **DB-AT-028/029 scale mismatch (SCALE-009, ARCH-SIM-CONSTRUCTION-001)** — **Mapped to:** [ARCH-SIM-CONSTRUCTION-001] (status: blocked_pending_environment) in docs/fix_plan.md. Stage A + `simulate_forward_once` now agree on calibrated intensity (SCALE-002), but `build_final_bragg_from_stage_a_telemetry` still produces bragg_after ≈1.0e-05 from raw simulator outputs ≈1.8e-14 while Stage A reports bragg_before ≈2.4e-01, driving DB-AT-028/029 chi²/pixel to ≈1e5 and median ROI CC to ≈−0.05. Proposed action: implement SCALE-009 in `dbex/refinement/reconstruction.py` by (1) multiplying each cold-path `sim.run()` panel by `sqrt_spot_scale` derived from `config.calibration_metadata['spot_scale_override']`, (2) treating `log_scale` as a delta so calibrated scale_factor=exp(delta) rather than exp(baseline+delta), (3) ensuring `N_cells` / `apply_calibration_n_cells` gating matches Stage A/mapping, and then (4) re-running ARCH-SIM-CONSTRUCTION-001 intensity probes plus DB-AT-028/029 selectors to confirm forward parity before any further reconstruction tuning.
  - 2025-12-13T190000Z: Scheduling Phase C.9 baseline probe (plans/active/ARCH-SIM-CONSTRUCTION-001/bin/compare_stage_a_baseline.py) to capture Stage A telemetry vs reconstructed `bragg_before` metrics under `plans/active/ARCH-SIM-CONSTRUCTION-001/reports/2025-12-13T190000Z/` before attempting additional fixes.
  - 2025-12-15T235500Z: Galph planned Phase C.10 to scale the cached Stage A zero-iteration stack (and telemetry masked means) so DB-AT-028/029 gate against the calibrated intensity instead of the pre-scale tensor; artifacts will land under `plans/active/ARCH-SIM-CONSTRUCTION-001/reports/2025-12-16T010000Z/`.
IMPORTANT:NEW
