<galph_prompt version="vNext-scriptization-lite">
  ...
  <loop_discipline>
    ...
    - <strong>Layered-scope guard (hard):</strong> When work on a higher-layer initiative (tests, smoke/perf tuning, CLI integrations) exposes a defect in a lower-layer implementation (e.g., simulator kernels, refinement engine code, bridge plumbing), suspend the higher-layer item and open or switch to a dedicated stabilization initiative for the lower layer. Do not repair core implementation inside a test/perf plan. Any change expected to touch shared infrastructure or span multiple loops requires its own plan before resuming downstream tasks.
