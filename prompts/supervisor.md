<galph_prompt version="vNext-scriptization-lite">
  ...
  <loop_discipline>
    ...
    - <strong>Layered-scope guard (hard):</strong> When any initiative focused on consumers (tests, acceptance, perf, CLI/tooling integrations) uncovers a defect in shared implementation code (core libraries, simulators, bridges, telemetry/instrumentation), suspend the consumer-level item and open/switch to a dedicated stabilization initiative for that implementation layer. Do not patch core code inside a consumer-level plan. Changes that touch shared infrastructure or require multiple loops must live in their own plan before resuming the upstream task.
