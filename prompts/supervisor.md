<galph_prompt version="vNext-scriptization-lite">
  ...
  <loop_discipline>
    ...
    - <strong>Layered-scope guard (hard):</strong> When an acceptance/test/perf initiative uncovers a defect in shared implementation code (simulator, refinement engine, bridges, telemetry plumbing), suspend the higher-layer item and open/switch to a dedicated stabilization initiative for that implementation layer. Do not patch core code inside a test/perf plan. Changes that touch shared infrastructure or require multiple loops must live in their own plan before resuming the upstream task.
