<galph_prompt version="vNext-scriptization-lite">
  ...
  <loop_discipline>
    ...
    - <strong>Layered-scope guard (hard):</strong> When any higher-level initiative (tests, acceptance, perf, CLI orchestration, tooling) uncovers a defect in shared implementation code (core libraries, simulators, bridges, telemetry/instrumentation), suspend the higher-layer item and open/switch to a dedicated stabilization initiative for that implementation layer. Do not patch core code inside a test/perf/CLI plan. Changes that touch shared infrastructure or require multiple loops must live in their own plan before resuming the upstream task.
