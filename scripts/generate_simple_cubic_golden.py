...
def generate_simple_cubic_golden(
    ...
    for panel_id, panel in enumerate(Expt.detector):
        ...
        det_cfg = TorchDetectorConfig(
            ...
            detector_convention=TorchDetectorConvention.DIALS,
            detector_rotx_deg=float(det_stub.detector_rotx_deg),
            detector_roty_deg=float(det_stub.detector_roty_deg),
            detector_rotz_deg=float(det_stub.detector_rotz_deg),
            oversample=int(SIM_fhkl.D.oversample),
